...
use Dompdf\Exception\Image_Exception;
use Dompdf\Helpers;
use Dompdf\Options;
/**
...
class Cache
{
     ...
     */
    public static function resolve_url($url, $protocol, $host, $base_path, Options $options): array
    {
        $full_url = null;
        $tempfile = null;
        $resolved_url = null;
        ...
        $message = null;
        try {
            $full_url = Helpers::build_url($protocol, $host, $base_path, $url, $options->get_chroot());
            if ($full_url === null) {
                throw new Image_Exception("Unable to parse image URL {$url}.", E_WARNING);
            }
            $parsed_url = Helpers::explode_url($full_url);
            $protocol = strtolower($parsed_url['protocol']);
            $is_data_uri = strpos($protocol, 'data:') === 0;
            $allowed_protocols = $options->get_allowed_protocols();
            if (!array_key_exists($protocol, $allowed_protocols)) {
                throw new Image_Exception("Permission denied on {$url}. The communication protocol is not supported.", E_WARNING);
            }
            foreach ($allowed_protocols[$protocol]['rules'] as $rule) {
                [$result, $message] = $rule($full_url);
                if (!$result) {
                    throw new Image_Exception("Error loading {$url}: {$message}", E_WARNING);
                }
            }
            if ($protocol === 'file://') {
                ...
                if (($resolved_url = @tempnam($tmp_dir, 'ca_dompdf_img_')) === false) {
                    throw new Image_Exception('Unable to create temporary image in ' . $tmp_dir, E_WARNING);
                }
                ...
                if ($is_data_uri) {
                    ...
                    [$image, $http_response_header] = Helpers::get_file_content($full_url, $options->get_http_context());
                }
                // Image not found or invalid
                if ($image === null) {
                    $msg = $is_data_uri ? 'Data-URI could not be parsed' : 'Image not found';
                    throw new Image_Exception($msg, E_WARNING);
                }
            ...
            if (!is_readable($resolved_url) || !filesize($resolved_url)) {
                throw new Image_Exception('Image not readable or empty', E_WARNING);
            }
            [$width, $height, $type] = Helpers::dompdf_getimagesize($resolved_url, $options->get_http_context());
            if (($width && $height && in_array($type, ['gif', 'png', 'jpeg', 'bmp', 'svg', 'webp'], true)) === false) {
                throw new Image_Exception('Image type unknown', E_WARNING);
            }
            if ($type === 'svg') {
                $parser = xml_parser_create('utf-8');
                xml_parser_set_option($parser, XML_OPTION_CASE_FOLDING, false);
                xml_set_element_handler($parser, function ($parser, $name, $attributes) use ($options, $parsed_url, $full_url): void {
                    if (strtolower($name) === 'image') {
                        if (!\array_key_exists($full_url, self::$svg_refs)) {
                            self::$svg_refs[$full_url] = [];
                        }
                        ...
                        foreach ($urls as $url) {
                            ...
                            $inner_full_url = Helpers::build_url($parsed_url['protocol'], $parsed_url['host'], $parsed_url['path'], $url, $options->get_chroot());
                            if (empty($inner_full_url)) {
                                continue;
                            }
                            self::detect_circular_ref($full_url, $inner_full_url);
                            self::$svg_refs[$full_url][] = $inner_full_url;
                            [$resolved_url, $type, $message] = self::resolve_url($url, $parsed_url['protocol'], $parsed_url['host'], $parsed_url['path'], $options);
                            if (!empty($message)) {
                                throw new Image_Exception("This SVG document references a restricted resource. {$message}", E_WARNING);
                            }
                        }
                    }
                }, null);
            ...
            Helpers::record_warnings($e->get_code(), $e->get_message() . " \n {$url}", $e->get_file(), $e->get_line());
            if ($full_url !== null) {
                self::$_cache[$full_url] = $resolved_url;
            }
        }
    ...
    public static function detect_circular_ref(string $src, string $target): void
    {
        if (!\array_key_exists($target, self::$svg_refs)) {
            return;
        }
        foreach (self::$svg_refs[$target] as $ref) {
            if ($ref === $src) {
                throw new Image_Exception('Circular external SVG image reference detected.', E_WARNING);
            }
            self::detect_circular_ref($src, $ref);
        }
    }
    /**
     ...
     */
    public static function add_temp_image(string $file_path, string $temp_path, string $key = 'default'): void
    {
        if (!isset(self::$temp_images[$file_path])) {
            self::$temp_images[$file_path] = [];
        }
        self::$temp_images[$file_path][$key] = $temp_path;
    }
    /**
     ...
     */
    public static function get_temp_image(string $file_path, string $key = 'default'): ?string
    {
        return self::$temp_images[$file_path][$key] ?? null;
    }
    /**
     * Unlink all cached images (i.e. temporary images either downloaded
     * or converted) except for the bundled "broken image"
     */
    public static function clear(bool $debug_png = false): void
    {
        ...
        foreach (self::$temp_images as $versions) {
    ...
    public static function is_broken($url): bool
    {
