 ...
 * @package dompdf
 */
class Page extends Abstract_Frame_Decorator
{
    /**
     ...
     */
    public function in_nested_table(): bool
    {
    ...
    public function check_forced_page_break(Frame $frame): bool
    {
        ...
        if ($this->_page_full || $frame->get_node()->node_name === 'body') {
            return false;
        }
        ...
        if (($frame->is_block_level() || $style->display === 'table-row') && in_array($style->page_break_before, $page_breaks, true)) {
        ...
        while ($prev && ($prev->is_text_node() && $prev->get_node()->node_value === '' || $prev->get_node()->node_name === 'bullet')) {
        ...
        if ($prev && ($prev->is_block_level() || $prev->get_style()->display === 'table-row') && !$prev->get_style()->is_absolute()) {
            ...
            while ($prev_last_child && ($prev_last_child->is_text_node() && $prev_last_child->get_node()->node_value === '' || $prev_last_child->get_node()->node_name === 'bullet')) {
                $prev_last_child = $prev_last_child->get_prev_sibling();
            }
            if ($prev_last_child && $prev_last_child->is_block_level() && in_array($prev_last_child->get_style()->page_break_after, $page_breaks, true)) {
                $frame->split(null, true, true);
     ...
     */
    protected function has_gap(float $child_pos, Frame $frame): bool
    {
        $style = $frame->get_style();
        $cbw = $frame->get_containing_block('w');
        $content_edge = $frame->get_position('y') + (float) $style->length_in_pt([$style->margin_top, $style->border_top_width, $style->padding_top], $cbw);
        return Helpers::length_greater($child_pos, $content_edge) && Helpers::length_less_or_equal($content_edge, $this->bottom_page_edge);
    }
    /**
    ...
    {
        Helpers::dompdf_debug('page-break', '_page_break_allowed(' . $frame->get_node()->node_name . ')');
        ...
        if ($frame->is_inline_level()) {
            ...
            $prev = $frame->get_prev_sibling();
            while ($prev && ($prev->is_text_node() && trim($prev->get_node()->node_value) == '')) {
                $prev = $prev->get_prev_sibling();
            }
            if ($block_parent->get_node()->node_name === 'body' && !$prev) {
                // We are the body's first child
                Helpers::dompdf_debug('page-break', "Body's first child.");
                return false;
            }
            ...
            if ($display === 'table-row') {
                ...
                if (!$prev) {
                    ...
                    if ($prev_group && in_array($prev_group->get_style()->display, Table::ROW_GROUPS, true)) {
    ...
    {
        if ($this->_page_full || $frame->_already_pushed || $frame->is_text_node() && $frame->get_node()->node_value === '') {
            return false;
        }
        $p = $frame;
        do {
        ...
        if (Helpers::length_less_or_equal($max_y, $this->bottom_page_edge)) {
            // no: do nothing
            return false;
        }
        ...
        while ($iter) {
            ...
            while ($next && $next->is_text_node() && $next->get_node()->node_value === '') {
     ...
     */
    public function add_floating_frame(Frame $frame): void
    {
     ...
     * @param $key
     */
    public function remove_floating_frame($key): void
    {
