...
class Cellmap
{
     ...
     * @return array[]
     */
    protected function get_resolved_border(Abstract_Frame_Decorator $frame): array
    {
        $key = $frame->get_id();
    ...
    public function add_frame(Frame $frame): void
    {
        ...
        if ($frame === $this->_table || $display === 'table-row' || in_array($display, Table_Frame_Decorator::ROW_GROUPS, true)) {
        ...
        $colspan = max((int) $node->get_attribute('colspan'), 1);
        $rowspan = max((int) $node->get_attribute('rowspan'), 1);
