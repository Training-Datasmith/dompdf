...
class Color
{
    ...
    public static function parse($color)
    {
        ...
        if (isset(self::$css_color_names[$color])) {
            return $cache[$color] = self::get_array(self::$css_color_names[$color]);
        }
        ...
        if (mb_substr($color, 0, 1) === '#') {
            ...
            // #rgb format
            if ($length === 4) {
                return $cache[$color] = self::get_array($color[1] . $color[1] . $color[2] . $color[2] . $color[3] . $color[3]);
            }
            // #rgba format
            if ($length === 5) {
                ...
                return $cache[$color] = self::get_array($color[1] . $color[1] . $color[2] . $color[2] . $color[3] . $color[3], $alpha);
            }
            // #rrggbb format
            if ($length === 7) {
                return $cache[$color] = self::get_array(mb_substr($color, 1, 6));
            }
            // #rrggbbaa format
            if ($length === 9) {
                ...
                return $cache[$color] = self::get_array(mb_substr($color, 1, 6), $alpha);
            }
            return null;
        }
        ...
        if (mb_substr($color, 0, 4) === 'rgb(' || mb_substr($color, 0, 5) === 'rgba(') {
            ...
            return $cache[$color] = self::get_array(vsprintf('%02X%02X%02X', $triplet), $alpha);
        }
        // cmyk( c,m,y,k ) format
        ...
        if (mb_substr($color, 0, 5) === 'cmyk(') {
            ...
            return $cache[$color] = self::get_array($values);
        }
     ...
     * @param float $alpha
     */
    public static function get_array($color, $alpha = 1.0): array
    {
