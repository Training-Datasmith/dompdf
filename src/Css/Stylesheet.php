...
class Stylesheet
{
     ...
     * @var string
     */
    public const PATTERN_CSS_STRING = '(?<CSS_STRING>(?<CSS_STRING_QUOTE>[\'"])(?<CSS_STRING_VALUE>.*?)(?<!\\\\)\g{CSS_STRING_QUOTE})';
    /**
     * RegEx pattern representing the CSS var() function
     *
     * @var string
     */
    public const PATTERN_CSS_VAR_FN = "var\\((([^()]|(?R))*)\\)";
    /**
     * RegEx pattern representing the CSS url() function
     *
     * @var string
     */
    public const PATTERN_CSS_URL_FN = '(?<CSS_URL_FN>url\(\s*(?<CSS_URL_FN_QUOTE>[\'"]?)(?<CSS_URL_FN_VALUE>.*?)(?(CSS_URL_FN_QUOTE)(?<!\\\\)\g{CSS_URL_FN_QUOTE})\s*\))';
    /**
     * RegEx pattern representing the CSS local() function
     *
     * @var string
     */
    public const PATTERN_CSS_LOCAL_FN = '(?<CSS_LOCAL_FN>local\(\s*(?<CSS_LOCAL_FN_QUOTE>[\'"]?)(?<CSS_LOCAL_FN_VALUE>.*?)(?(CSS_LOCAL_FN_QUOTE)(?<!\\\\)\g{CSS_LOCAL_FN_QUOTE})\s*\))';
    /**
     * RegEx pattern representing a CSS media query
     *
     * @var string
     */
    public const PATTERN_MEDIA_QUERY = '(?<CSS_MEDIA_QUERY>(?:(?:(?:(?<CSS_MEDIA_QUERY_OP>only|not)\s+)?(?<CSS_MEDIA_QUERY_TYPE>all|aural|bitmap|braille|dompdf|embossed|handheld|paged|print|projection|screen|speech|static|tty|tv|visual))|(?:\(\s*(?<CSS_MEDIA_QUERY_FEATURE>(?:(?:(?:min|max)-)?(?:width|height))|orientation|[^:]*?)\s*(?:\:\s*(?<CSS_MEDIA_QUERY_CONDITION>.*?)\s*)?\))))';
    /*
    ...
    {
        $this->_dompdf = $dompdf;
        $this->set_font_metrics($dompdf->get_font_metrics());
        $this->_styles = [];
        $this->_loaded_files = [];
        $script = __FILE__;
     ...
     */
    public function set_protocol(string $protocol): void
    {
     ...
     */
    public function create_style(): Style
    {
     ...
     */
    public function add_style(string $key, Style $style): void
    {
     ...
     * @param int $origin
     */
    public function load_css(&$css, $origin = self::ORIG_AUTHOR): void
    {
        if ($origin) {
     ...
     * @param int $origin
     */
    public function load_css_file($file, $origin = self::ORIG_AUTHOR): void
    {
        if ($origin) {
            $this->_current_origin = $origin;
        }
        ...
        if ($file !== $this->get_default_stylesheet()) {
        ...
        if (strpos($file, 'data:') === 0) {
            ...
            [$css, $http_response_header] = Helpers::get_file_content($file, $this->_dompdf->get_http_context());
            $good_mime_type = true;
    ...
    protected function specificity(string $selector, int $origin = self::ORIG_AUTHOR): int
    {
        ...
        if ($this->_dompdf->get_options()->get_debug_css()) {
            /*DEBUGCSS*/
            print "<pre>\n";
            /*DEBUGCSS*/
     ...
     *
     */
    protected function selector_to_xpath(string $selector, bool $first_pass = false): ?array
    {
                ...
                case ':':
                    ...
                    switch ($tok) {
                        ...
                        case 'nth-child':
                            ...
                            $condition = $this->selector_an_plus_b($nth, $position);
                            $query .= "[{$condition}]";
                            break;
                        ...
                        case 'matches':
                            $p = $i + 1;
                            $match_list = trim(mb_substr($selector, $p, mb_strpos($selector, ')', $i, 'UTF-8') - $p, 'UTF-8'));
                            // Tag names are case-insensitive
                            $elements = array_map('trim', explode(',', strtolower($match_list)));
     ...
     */
    protected function selector_an_plus_b(string $expr, string $position): string
    {
        // odd
        if ($expr === 'odd') {
     ...
     * of this class.
     */
    public function apply_styles(Frame_Tree $tree): void
    {
        ...
        $styles = [];
        $xp = new Domx_Path($tree->get_dom());
        $DEBUGCSS = $this->_dompdf->get_options()->get_debug_css();
        // Add generated content
        foreach ($this->_styles as $selector => $selector_styles) {
            if (strpos($selector, ':before') === false && strpos($selector, ':after') === false) {
                continue;
            }
            $query = $this->selector_to_xpath($selector, true);
            if ($query === null) {
            ...
            foreach ($selector_styles as $style) {
                foreach ($nodes as $node) {
                    ...
                    foreach (array_keys($query['pseudo_elements'], true, true) as $pos) {
                        ...
                        if ($node->has_attribute("dompdf_{$pos}_frame_id")) {
                            continue;
                        }
                        ...
                        if ($single instanceof Url) {
                            ...
                            $new_node = $node->owner_document->create_element('img_generated');
                            $new_node->set_attribute('src', $src);
                        } else {
                            $new_node = $node->owner_document->create_element('dompdf_generated');
                        }
                        $new_node->set_attribute($pos, $pos);
                        $new_frame_id = $tree->insert_node($node, $new_node, $pos);
                        $node->set_attribute("dompdf_{$pos}_frame_id", $new_frame_id);
                    }
                }
            }
        }
        ...
        foreach ($this->_styles as $selector => $selector_styles) {
            ...
            foreach ($selector_styles as $style) {
                ...
                foreach ($nodes as $node) {
                    ...
                    $id = $node->get_attribute('frame_id');
        ...
        foreach ($tree as $frame) {
            ...
            if (($str = $frame->get_node()->get_attribute(Attribute_Translator::$_style_attr)) !== '') {
                $styles[$id][self::SPEC_NON_CSS][] = $this->_parse_properties($str);
            }
            // Locate any additional style attributes
            if (($str = $frame->get_node()->get_attribute('style')) !== '') {
            ...
            if (isset($styles[$id])) {
                ...
                if ($DEBUGCSS) {
                    $debug_nodename = $frame->get_node()->node_name;
                    print "<pre>\n{$debug_nodename} [\n";
                ...
                $acceptedmedia[] = $this->_dompdf->get_options()->get_default_media_type();
                            ...
                            foreach ($media_queries as $media_query_group) {
                                foreach ($media_query_group as $media_query) {
                                    ...
                                    if ($negate xor !$feature_match) {
                                        continue 2;
                                        // failed query match, move to the next grouping
                                    }
                                }
    ...
    private function _parse_css($str, array $media_queries = []): void
    {
        ...
        $accepted_media[] = $this->_dompdf->get_options()->get_default_media_type();
    ...
    public function resolve_url($val, $resolve_blobs = false): string
    {
        ...
        if ($val === null || $val === '' || strcasecmp($val, 'none') === 0) {
            ...
            $path = Helpers::build_url($this->_protocol, $this->_base_host, $this->_base_path, $url, $this->_dompdf->get_options()->get_chroot());
            if ($path === null) {
                $path = 'none';
            }
        } else {
    ...
    private function _parse_font_face(string $str): void
    {
        ...
        foreach ($sources as $source) {
            ...
            if ($urlfn !== '' && $format === 'truetype') {
                ...
                $source_info = ['uri' => $urlfn, 'format' => $format, 'path' => $url];
        ...
        foreach ($valid_sources as $valid_source) {
            if ($this->font_metrics->register_font($style, $valid_source['path'], $this->_dompdf->get_http_context())) {
                break;
            }
        }
    }
    ...
    public function parse_string(string $string): string
    {
        ...
        return preg_replace_callback("/\\\\([0-9a-fA-F]{1,6})\\s?/", function (array $matches) {
     ...
     * @return string
     */
    public function get_default_stylesheet()
    {
        $options = $this->_dompdf->get_options();
        $root_dir = realpath($options->get_root_dir());
        return Helpers::build_url('file://', '', $root_dir, $root_dir . self::DEFAULT_STYLESHEET, $options->get_chroot());
    }
    /**
     * @return $this
     */
