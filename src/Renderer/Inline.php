 ...
 * @package dompdf
 */
class Inline extends Abstract_Renderer
{
    public function render(Frame $frame): void
    {
        ...
        if ($options->get_debug_layout() && $options->get_debug_layout_inline()) {
            $this->debug_layout($border_box, 'blue');
            if ($options->get_debug_layout_padding_box()) {
                $padding_box = [$x + $style->border_left_width, $y + $style->border_top_width, $w - $style->border_left_width - $style->border_right_width, $h - $style->border_top_width - $style->border_bottom_width];
                $this->debug_layout($padding_box, 'blue', [0.5, 0.5]);
            }
        }
    }
    protected function get_child_size(Frame $frame): array
    {
        ...
        foreach ($frame->get_children() as $child) {
            ...
            if ($child->get_node()->node_value === ' ' && $child->get_prev_sibling() && !$child->get_next_sibling()) {
                break;
            }
