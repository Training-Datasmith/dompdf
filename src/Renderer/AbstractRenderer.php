...
abstract class Abstract_Renderer
{
    ...
    protected function _background_image(string $url, float $x, float $y, float $width, float $height, Style $style): void
    {
        ...
        list($img, $type, ) = Cache::resolve_url($url, $sheet->get_protocol(), $sheet->get_host(), $sheet->get_base_path(), $this->_dompdf->get_options());
        ...
        [$img_w, $img_h] = Helpers::dompdf_getimagesize($img, $this->_dompdf->get_http_context());
        if ($img_w == 0 || $img_h == 0) {
            return;
        }
        ...
        [$img_w, $img_h] = $this->_resize_background_image($img_w, $img_h, $bg_width, $bg_height, $style->background_size, $dpi);
        ...
        $cpdf_key = $img . '_' . $key;
        $tmp_file = Cache::get_temp_image($img, $key);
        $cached = $this->_canvas instanceof CPDF && $this->_canvas->get_cpdf()->image_iscached($cpdf_key) || $tmp_file !== null && file_exists($tmp_file);
        if (!$cached) {
            ...
            if ($img_w != $org_img_w || $img_h != $org_img_h) {
                $new_src = imagecreatetruecolor($img_w, $img_h);
                imagealphablending($new_src, false);
                imagesavealpha($new_src, true);
                imagecopyresampled($new_src, $src, 0, 0, 0, 0, $img_w, $img_h, imagesx($src), imagesy($src));
            ...
            if ($cpdf_from_gd && $this->_canvas instanceof CPDF) {
                ...
                Cache::add_temp_image($img, $tmp_file, $key);
            }
        } else {
            $bg = null;
            $cpdf_from_gd = $tmp_file === null;
        }
        if ($this->_dompdf->get_options()->get_debug_png()) {
        ...
        if ($cpdf_from_gd && $this->_canvas instanceof CPDF) {
            ...
            $this->_canvas->get_cpdf()->add_image_png($bg, $cpdf_key, $x, $this->_canvas->get_height() - $y - $height, $width, $height);
            if (PHP_MAJOR_VERSION < 8) {
                imagedestroy($bg);
            }
        } else {
     ...
     * @return float[]
     */
    protected function _resize_background_image(float $img_width, float $img_height, float $container_width, float $container_height, $bg_resize, int $dpi): array
    {
     ...
     *
     */
    protected function dash_pattern(string $style, float $width, float $length): array
    {
        if ($style === 'dashed') {
            $w = 3 * $width;
            if ($length < $w) {
    ...
    protected function _border_line($x, $y, $length, $color, $widths, $side, $corner_style = 'bevel', string $pattern_name = 'none', $r1 = 0, $r2 = 0)
    {
        ...
        $corner_clip = $corner_style === 'bevel' && ($r1 < $width || $r2 < $width);
        $line_length = $length - $r1 - $r2;
        [$pattern, $cap] = $this->dash_pattern($pattern_name, $width, $line_length);
        // Determine arc border radius for corner arcs
        $half_width = $width / 2;
        $ar1 = max($r1 - $half_width, 0);
        $ar2 = max($r2 - $half_width, 0);
        ...
        // occasionally
        $dl = $cap === 'round' ? $half_width : 0;
        if ($cap === 'round' && $ar1 > 0) {
            $adj1 -= rad2deg(asin($half_width / $ar1));
        }
        if ($cap === 'round' && $ar2 > 0) {
            $adj2 -= rad2deg(asin($half_width / $ar2));
        }
        switch ($side) {
            case 'top':
                if ($corner_clip) {
     ...
     * with `name` attribute.
     */
    protected function add_named_dest(Dom_Element $node): void
    {
     ...
     */
    protected function add_hyperlink(Dom_Element $node, array $border_box): void
    {
        if ($node->node_name === 'a' && ($href = $node->get_attribute('href')) !== '') {
            [$x, $y, $w, $h] = $border_box;
            $dompdf = $this->_dompdf;
            $href = Helpers::build_url($dompdf->get_protocol(), $dompdf->get_base_host(), $dompdf->get_base_path(), $href, $dompdf->get_options()->get_chroot()) ?? $href;
     ...
     */
    protected function debug_layout(array $box, $color = 'red', array $style = []): void
    {
