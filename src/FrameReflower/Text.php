...
use Dompdf\Frame_Decorator\Inline as InlineFrameDecorator;
use Dompdf\Frame_Decorator\Text as TextFrameDecorator;
use Dompdf\Helpers;
/**
 * Reflows text frames.
 *
 * @package dompdf
 */
class Text extends Abstract_Frame_Reflower
{
    /**
     * PHP string representation of HTML entity <shy>
     */
    public const SOFT_HYPHEN = "­";
    /**
    ...
    public function __construct(Text_Frame_Decorator $frame, Font_Metrics $font_metrics)
    {
        parent::__construct($frame);
        $this->set_font_metrics($font_metrics);
    }
    /**
     ...
     * @return int|false
     */
    protected function line_break(string $text, Block_Frame_Decorator $block, bool $nowrap = false)
    {
        ...
        $text_width = $font_metrics->get_text_width($visible_text, $font, $size, $word_spacing, $letter_spacing);
        ...
        if (Helpers::length_less_or_equal($frame_width, $available_width)) {
            return false;
        }
        $force_first = $current_line->left == 0 && $current_line->right == 0 && $current_line->is_empty();
        if ($nowrap) {
        ...
        $width = 0.0;
        $str = '';
        $space_width = $font_metrics->get_text_width(' ', $font, $size, $word_spacing, $letter_spacing);
        $shy_width = $font_metrics->get_text_width(self::SOFT_HYPHEN, $font, $size);
        // @todo support <wbr>
        for ($i = 0; $i < $wc; $i += 2) {
            ...
            $word_width = $font_metrics->get_text_width($word, $font, $size, $word_spacing, $letter_spacing);
            $used_width = $width + $word_width + $mbp_width;
            if ($used_width > 0 && Helpers::length_greater($used_width, $available_width)) {
        ...
        if ($force_first && $width === 0.0) {
            ...
            if ($break_word) {
                ...
                for ($j = 0; $j < $len; $j++) {
                    ...
                    $w = $font_metrics->get_text_width($s . $c, $font, $size, $word_spacing, $letter_spacing);
                    if (Helpers::length_greater($w, $available_width)) {
                        break;
                    }
                    $s .= $c;
                }
     ...
     *         should be stopped.
     */
    protected function layout_line(Block_Frame_Decorator $block): ?bool
    {
        $frame = $this->_frame;
        $style = $frame->get_style();
        $current_line = $block->get_current_line_box();
        ...
        switch ($white_space) {
            ...
            case 'pre-wrap':
                ...
                $first_line = $hard_split !== false ? mb_substr($text, 0, $hard_split, 'UTF-8') : $text;
        ...
        if ($split === 0) {
            ...
            $child = $frame;
            $p = $child->get_parent();
            while ($p instanceof Inline_Frame_Decorator && !$child->get_prev_sibling()) {
                $child = $p;
                $p = $p->get_parent();
            }
        ...
        if ($split !== false && $split < mb_strlen($text, 'UTF-8')) {
            ...
            $t = $frame->get_text();
            $shy_position = mb_strpos($t, self::SOFT_HYPHEN, 0, 'UTF-8');
            if (false !== $shy_position && $shy_position < mb_strlen($t, 'UTF-8') - 1) {
    ...
    public function reflow(?Block_Frame_Decorator $block = null): void
    {
        ...
        $size = $style->font_size;
        $font = $style->font_family;
        $font_height = $this->get_font_metrics()->get_font_height($font, $size);
    ...
    public function get_min_max_width(): array
    {
        ...
        switch ($style->white_space) {
            ...
            case 'pre-wrap':
                ...
                if ($style->overflow_wrap === 'anywhere') {
                    ...
                    $min = $font_metrics->get_text_width($char, $font, $size, $word_spacing, $letter_spacing);
                } else {
                    // Find the longest word
                    $words = preg_split(self::$_wordbreak_pattern, $text, -1, PREG_SPLIT_DELIM_CAPTURE);
                    $lengths = array_map(function (array $chunk) use ($font_metrics, $font, $size, $word_spacing, $letter_spacing) {
                        ...
                        return $font_metrics->get_text_width($word, $font, $size, $word_spacing, $letter_spacing);
                    }, array_chunk($words, 2));
                    $min = max($lengths);
                }
                break;
            case 'pre':
                ...
                array_walk($lines, function (&$chunked_text_width, string $chunked_text) use ($font_metrics, $font, $size, $word_spacing, $letter_spacing): void {
                    $chunked_text_width = $font_metrics->get_text_width($chunked_text, $font, $size, $word_spacing, $letter_spacing);
                });
                arsort($lines);
                $min = reset($lines);
                break;
            case 'nowrap':
                $min = $font_metrics->get_text_width($visible_text, $font, $size, $word_spacing, $letter_spacing);
                break;
        }
        ...
        switch ($style->white_space) {
            default:
            case 'normal':
                $max = $font_metrics->get_text_width($visible_text, $font, $size, $word_spacing, $letter_spacing);
                break;
            case 'pre-line':
            case 'pre-wrap':
                // Find the longest line
