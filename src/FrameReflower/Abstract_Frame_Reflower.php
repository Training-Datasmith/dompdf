...
use Dompdf\Frame_Decorator\Block;
/**
 * Base reflower class
 *
 ...
 * @package dompdf
 */
abstract class Abstract_Frame_Reflower
{
    /**
     ...
     */
    public function __construct(Abstract_Frame_Decorator $frame)
    {
        $this->_frame = $frame;
     ...
     */
    protected function position_relative(Abstract_Frame_Decorator $frame): void
    {
        $style = $frame->get_style();
    ...
    protected function resolve_content(): ?string
    {
        ...
        foreach ($content as $val) {
            if ($val instanceof String_Part) {
                $text .= $val->string;
            } elseif ($val instanceof Open_Quote) {
                // FIXME: Take quotation depth into account
                if ($quotes !== 'none' && isset($quotes[0][0])) {
                    $text .= $quotes[0][0];
                }
            } elseif ($val instanceof Close_Quote) {
                // FIXME: Take quotation depth into account
                if ($quotes !== 'none' && isset($quotes[0][1])) {
                    $text .= $quotes[0][1];
                }
            } elseif ($val instanceof No_Open_Quote) {
                // FIXME: Increment quotation depth
            } elseif ($val instanceof No_Close_Quote) {
                // FIXME: Decrement quotation depth
            } elseif ($val instanceof Attr) {
                $text .= $frame->get_parent()->get_node()->get_attribute($val->attribute);
    ...
    protected function _set_content(): void
    {
        ...
        if ($frame->get_node()->node_name === 'dompdf_generated') {
            $content = $this->resolve_content();
            if ($content !== null) {
                $node = $frame->get_node()->owner_document->create_text_node($content);
