...
use Dompdf\Frame_Decorator\Table_Cell as TableCellFrameDecorator;
 ...
 * @package dompdf
 */
class Block extends Abstract_Frame_Reflower
{
    // Minimum line width to justify, as fraction of available width
    public const MIN_JUSTIFY_WIDTH = 0.8;
    /**
    ...
    public function vertical_align(): void
    {
        ...
        foreach ($this->_frame->get_line_boxes() as $line) {
            ...
            foreach ($line->frames_to_align() as $frame) {
                $style = $frame->get_style();
                $is_inline_block = $style->display !== 'inline' && $style->display !== '-dompdf-list-bullet';
                $baseline = $font_metrics->get_font_baseline($style->font_family, $style->font_size);
                $y_offset = 0;
                ...
                if ($is_inline_block) {
                    ...
                    foreach ($line->get_frames() as $other) {
                        if ($other !== $frame && !($other->is_text_node() && $other->get_node()->node_value === '')) {
                            $skip = false;
                            break;
                        }
                    }
                    ...
                    $image_height_diff = $height * 0.8 - $margin_height;
                    $align = $frame->get_style()->vertical_align;
                    if (in_array($align, Style::VERTICAL_ALIGN_KEYWORDS, true)) {
                        switch ($align) {
                            case 'middle':
                                $y_offset = $image_height_diff / 2;
                                break;
                            case 'sub':
                            case 'text-bottom':
                            // FIXME: align bottom of image with the descender?
                            case 'bottom':
                                $y_offset = 0.3 * $height + $image_height_diff;
                                break;
                            case 'super':
                                $y_offset = -0.2 * $height + $image_height_diff;
                                break;
                            case 'text-top':
                            ...
                            case 'baseline':
                            default:
                                $y_offset = $image_height_diff;
                                break;
                        }
                    } else {
                        $y_offset = $baseline - (float) $style->length_in_pt($align, $style->font_size) - $margin_height;
                    }
                } else {
                    $parent = $frame->get_parent();
                    if ($parent instanceof Table_Cell_Frame_Decorator) {
                        $align = 'baseline';
                    } else {
    ...
    public function process_clear(Abstract_Frame_Decorator $child): void
    {
     ...
     * @param float $cb_w
     */
    public function process_float(Abstract_Frame_Decorator $child, $cb_x, $cb_w): void
    {
        $child_style = $child->get_style();
        $root = $this->_frame->get_root();
        // Handle "float"
        if ($child_style->float !== 'none') {
            ...
            } else if ($child_style->float === 'right') {
