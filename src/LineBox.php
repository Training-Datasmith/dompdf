...
use Dompdf\Frame_Decorator\List_Bullet;
use Dompdf\Frame_Decorator\Page;
use Dompdf\Frame_Reflower\Text as TextFrameReflower;
...
class Line_Box
{
    ...
    public function get_float_offsets(): void
    {
        ...
        foreach ($floating_frames as $child_key => $floating_frame) {
            ...
            if (self::$float_offset_anti_infinite_loop-- > 0 && $floating_frame->get_position('y') + $floating_frame->get_margin_height() >= $this->y && $block->get_position('x') + $block->get_margin_width() >= $floating_frame->get_position('x')) {
                if ($float === 'left') {
    ...
    {
        $this->_frames[] = $frame;
        if ($frame->get_positioner() instanceof Inline_Positioner) {
            $this->inline = true;
        }
    }
    /**
    ...
    {
        $last_index = count($this->_frames) - 1;
        ...
        foreach ($this->_frames as $f) {
            ...
            if ($f->get_positioner() instanceof Inline_Positioner) {
                $this->inline = true;
            }
        }
        $this->h = $h;
     ...
     */
    public function add_list_marker(List_Bullet $marker): void
    {
    ...
    public function trim_trailing_ws(): void
    {
        ...
        $last_frame = $this->_frames[$last_index];
        $reflower = $last_frame->get_reflower();
        if ($reflower instanceof Text_Frame_Reflower && !$last_frame->is_pre()) {
