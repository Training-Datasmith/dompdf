 ...
 * @package dompdf
 */
class Pdf_Lib implements Canvas
{
    /**
     ...
     * @var array
     */
    public static $native_fonts_to_pdf_lib = ['courier' => 'Courier', 'courier-bold' => 'Courier-Bold', 'courier-oblique' => 'Courier-Oblique', 'courier-boldoblique' => 'Courier-BoldOblique', 'helvetica' => 'Helvetica', 'helvetica-bold' => 'Helvetica-Bold', 'helvetica-oblique' => 'Helvetica-Oblique', 'helvetica-boldoblique' => 'Helvetica-BoldOblique', 'times' => 'Times-Roman', 'times-roman' => 'Times-Roman', 'times-bold' => 'Times-Bold', 'times-italic' => 'Times-Italic', 'times-bolditalic' => 'Times-BoldItalic', 'symbol' => 'Symbol', 'zapfdinbats' => 'ZapfDingbats', 'zapfdingbats' => 'ZapfDingbats'];
    /**
     * @var \Dompdf\Dompdf
     */
    protected $_dompdf;
    ...
    public function __construct($paper = 'letter', string $orientation = 'portrait', ?Dompdf $dompdf = null)
    {
        ...
        $options = $dompdf->get_options();
        $this->_pdf = new \Pdf_Lib();
        $license = $options->get_pdflib_license();
        if (strlen($license) > 0) {
            $this->set_pdf_lib_parameter('license', $license);
        }
        if ($this->get_pdf_lib_major_version() < 10) {
            $this->set_pdf_lib_parameter('textformat', 'utf8');
        }
        if ($this->get_pdf_lib_major_version() >= 7) {
            $this->set_pdf_lib_parameter('errorpolicy', 'return');
            ...
            $this->set_pdf_lib_parameter('fontwarning', 'false');
        }
        $search_path = [$options->get_font_dir(), $options->get_root_dir() . '/lib/fonts'];
        $this->_pdf->set_option('searchpath={{' . implode('} {', $search_path) . '}}');
        // fetch PDFLib version information for the producer field
        $this->_pdf->set_info('Producer Addendum', sprintf('%s + PDFLib %s', $dompdf->version, $this->get_pdf_lib_major_version()));
        ...
        if ($options->is_pdf_a_enabled()) {
            $icc_profile_path = $options->get_root_dir() . '/lib/res/sRGB2014.icc';
            $this->_pdf->load_iccprofile($icc_profile_path, 'usage=outputintent');
        }
    ...
    protected function _set_line_style($width, $cap, $join, $dash)
    {
        ...
        if ($this->get_pdf_lib_major_version() >= 9) {
            if (count($dash) > 1) {
    ...
    protected function _set_stroke_color(array $color)
    {
        ...
        if (isset($color[3])) {
            ...
            $type = 'rgb';
            [$c1, $c2, $c3, $c4] = [$color[0], $color[1], $color[2], 0];
        } else {
            $type = 'gray';
            [$c1, $c2, $c3, $c4] = [$color[0], $color[1], 0, 0];
        }
     ...
     * @param string $mode
     */
    public function _set_fill_opacity($opacity, $mode = 'Normal'): void
    {
     ...
     * @param string $mode
     */
    public function _set_stroke_opacity($opacity, $mode = 'Normal'): void
    {
        if ($mode === 'Normal' && isset($opacity)) {
     ...
     */
    public function _set_gstate($gstate_options): bool
    {
     ...
     */
    protected function _load_font($font, $encoding = null, string $options = '')
    {
        // Fix for PDFLib's case-sensitive font names
        $base_font = basename($font);
        $is_native_font = false;
        $lc_base_font = strtolower($base_font);
        if (isset(self::$native_fonts_to_pdf_lib[$lc_base_font])) {
            $base_font = self::$native_fonts_to_pdf_lib[$lc_base_font];
            $is_native_font = true;
        }
        ...
        $options .= ' autosubsetting=' . ($this->_dompdf->get_options()->get_is_font_subsetting_enabled() === false ? 'false' : 'true');
        if (is_null($encoding)) {
            // Unicode encoding is only available for the commerical
            // version of PDFlib and not PDFlib-Lite
            if (strlen($this->_dompdf->get_options()->get_pdflib_license()) > 0) {
                $encoding = 'unicode';
            } else {
                $encoding = 'auto';
            }
        ...
        if ($is_native_font) {
            $this->_fonts[$key] = $this->_pdf->load_font($base_font, $encoding, $options);
            return $this->_fonts[$key];
        }
        $font_outline = $this->get_pdf_lib_parameter('FontOutline', 1);
        if ($font_outline === '' || $font_outline < 0) {
            $families = $this->_dompdf->get_font_metrics()->get_font_families();
            foreach ($families as $files) {
                foreach ($files as $file) {
                    $face = basename($file);
                    $afm = null;
                    if (isset($this->_fonts_files[$face])) {
                        continue;
                    }
                    // Prefer ttfs to afms
                    ...
                    $this->_fonts_files[$face] = true;
                    if ($this->get_pdf_lib_major_version() >= 9) {
                        $this->set_pdf_lib_parameter('FontOutline', '{' . "{$face}={$outline}" . '}');
                    } else {
                        $this->set_pdf_lib_parameter('FontOutline', "\\{{$face}\\}=\\{{$outline}\\}");
                    }
                    if (is_null($afm)) {
                    ...
                    if ($this->get_pdf_lib_major_version() >= 9) {
                        $this->set_pdf_lib_parameter('FontAFM', '{' . "{$face}={$afm}" . '}');
                    } else {
                        $this->set_pdf_lib_parameter('FontAFM', "\\{{$face}\\}=\\{{$afm}\\}");
                    }
                }
    ...
    public function clipping_roundrectangle($x1, $y1, $w, $h, $r_tl, $r_tr, $r_br, $r_bl): void
    {
        ...
        $path = $this->_pdf->add_path_point($path, 0, 0 - $r_tl + $h, 'move', '');
        // line: left edge, bottom end
        $path = $this->_pdf->add_path_point($path, 0, 0 + $r_bl, 'line', '');
        // curve: bottom-left corner
        if ($r_bl > 0) {
            $path = $this->_pdf->add_path_point($path, 0 + $r_bl, 0, 'elliptical', "radius={$r_bl} clockwise=false");
        }
        // line: bottom edge, left end
        $path = $this->_pdf->add_path_point($path, 0 - $r_br + $w, 0, 'line', '');
        // curve: bottom-right corner
        if ($r_br > 0) {
            $path = $this->_pdf->add_path_point($path, 0 + $w, 0 + $r_br, 'elliptical', "radius={$r_br} clockwise=false");
        }
        // line: right edge, top end
        $path = $this->_pdf->add_path_point($path, 0 + $w, 0 - $r_tr + $h, 'line', '');
        // curve: top-right corner
        if ($r_tr > 0) {
            $path = $this->_pdf->add_path_point($path, 0 - $r_tr + $w, 0 + $h, 'elliptical', "radius={$r_tr} clockwise=false");
        }
        // line: top edge, left end
        $path = $this->_pdf->add_path_point($path, 0 + $r_tl, 0 + $h, 'line', '');
        // curve: top-left corner
        if ($r_tl > 0) {
            $path = $this->_pdf->add_path_point($path, 0, 0 - $r_tl + $h, 'elliptical', "radius={$r_tl} clockwise=false");
        }
    ...
    public function image($img, $x, $y, $w, $h, $resolution = 'normal'): void
    {
        $w = (int) $w;
        $h = (int) $h;
        $img_type = Cache::detect_type($img, $this->get_dompdf()->get_http_context());
        // Strip file:// prefix
    ...
    public function text($x, $y, $text, $font, $size, $color = [0, 0, 0], $word_spacing = 0, $char_spacing = 0, $angle = 0): void
    {
        if ($size == 0) {
            return;
        }
    ...
    public function get_font_baseline($font, $size): float
    {
        $ratio = $this->_dompdf->get_options()->get_font_height_ratio();
    ...
    protected function process_page_script(callable $callback): void
    {
        ...
        for ($p = 1; $p <= $this->_page_count; $p++) {
            ...
            $callback($p, $this->_page_count, $this, $font_metrics);
            $this->_pdf->suspend_page('');
        }
        $this->_pdf->resume_page('pagenumber=' . $this->_page_number);
    }
    ...
    public function stream($filename = 'document.pdf', $options = []): void
    {
        ...
        if ($options['compress']) {
            $this->set_pdf_lib_value('compress', 6);
        } else {
            $this->set_pdf_lib_value('compress', 0);
        }
        $this->_close();
        $data = '';
        if (self::$IN_MEMORY) {
        ...
        if (self::$IN_MEMORY) {
            ...
            fclose($fh);
            //debugpng
            if ($this->_dompdf->get_options()->get_debug_png()) {
                print '[pdflib stream unlink ' . $this->_file . ']';
            }
            if (!$this->_dompdf->get_options()->get_debug_keep_temp()) {
                unlink($this->_file);
            }
            $this->_file = null;
            unset($this->_file);
     ...
     * @return mixed
     */
    protected function get_pdf_lib_parameter($keyword, $optlist = '')
    {
        if ($this->get_pdf_lib_major_version() >= 9) {
    ...
    protected function set_pdf_lib_parameter(string $keyword, string $value): bool
    {
        if ($this->get_pdf_lib_major_version() >= 9) {
     ...
     * @return mixed
     */
    protected function get_pdf_lib_value($keyword, $optlist = '')
    {
        if ($this->get_pdf_lib_major_version() >= 9) {
            return $this->get_pdf_lib_parameter($keyword, $optlist);
        }
     ...
     * @return mixed
     */
    protected function set_pdf_lib_value($keyword, $value)
    {
        if ($this->get_pdf_lib_major_version() >= 9) {
            return $this->set_pdf_lib_parameter($keyword, $value);
        }
     ...
     * @return int
     */
    protected function get_pdf_lib_major_version()
    {
...
Pdf_Lib::$PAPER_SIZES = CPDF::$PAPER_SIZES;