...
class GD implements Canvas
{
    ...
    public function __construct($paper = 'letter', string $orientation = 'portrait', ?Dompdf $dompdf = null, float $aa_factor = 1.0, array $bg_color = [1, 1, 1, 0])
    {
        ...
        $this->dpi = $this->get_dompdf()->get_options()->get_dpi();
        if ($aa_factor < 1) {
            $aa_factor = 1;
        }
     ...
     * @param float $length
     */
    protected function _upscale($length): float
    {
     ...
     * @param float $length
     */
    protected function _downscale($length): float
    {
        return round($length / $this->dpi * 72 / $this->_aa_factor);
    }
    protected function convert_style(array $style, int $color, int $width): array
    {
        $gd_style = [];
        if (count($style) === 1) {
            $style[] = $style[0];
        ...
        foreach ($style as $index => $s) {
            ...
            for ($i = 0; $i < $d; $i++) {
                for ($j = 0; $j < $width; $j++) {
                    $gd_style[] = $index % 2 === 0 ? $color : IMG_COLOR_TRANSPARENT;
                }
            }
        }
        return $gd_style;
    ...
    public function line($x1, $y1, $x2, $y2, $color, $width, $style = [], $cap = 'butt'): void
    {
        ...
        if (is_array($style) && count($style) > 0) {
            $gd_style = $this->convert_style($style, $c, $width);
            if (!empty($gd_style)) {
    ...
    {
        $this->get_dompdf()->get_options()->set_dpi(72);
    }
    public function restore(): void
    {
        $this->get_dompdf()->get_options()->set_dpi($this->dpi);
    }
    ...
    public function text($x, $y, $text, $font, $size, $color = [0, 0, 0], $word_spacing = 0.0, $char_spacing = 0.0, $angle = 0.0): void
    {
        // Scale by the AA factor and DPI
        $x = $this->_upscale($x);
        $y = $this->_upscale($y);
    ...
    private function get_char_map(string $font)
    {
        static $unicode_char_map_tables = [];
        if (isset($unicode_char_map_tables[$font])) {
            return $unicode_char_map_tables[$font];
        }
        ...
        if (file_exists($cache_name)) {
            ...
            return $unicode_char_map_tables[$font] = $char_map;
        }
        $char_map = [];
    ...
    public function font_supports_char(string $font, string $char): bool
    {
        ...
        $font = $this->get_ttf_file($font);
        $char_map = $this->get_char_map($font);
    ...
    public function get_ttf_file($font): string
    {
        ...
        if (!file_exists($font)) {
            ...
            $font = $font_metrics->get_font($this->_dompdf->get_options()->get_default_font()) . '.ttf';
            if (!file_exists($font)) {
                if (strpos($font, 'mono')) {
                    $font = $font_metrics->get_font('DejaVu Mono') . '.ttf';
                } elseif (strpos($font, 'sans') !== false) {
                    $font = $font_metrics->get_font('DejaVu Sans') . '.ttf';
                } elseif (strpos($font, 'serif')) {
                    $font = $font_metrics->get_font('DejaVu Serif') . '.ttf';
                } else {
                    $font = $font_metrics->get_font('DejaVu Sans') . '.ttf';
                }
            }
    ...
    public function open_object(): void
    {
        // N/A
    }
