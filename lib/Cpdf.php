...
use Font_Lib\Binary_Stream;
use Font_Lib\Exception\Font_Not_Found_Exception;
use Font_Lib\Font;
class Cpdf
{
    public const PDF_VERSION = '1.7';
    public const ACROFORM_SIG_SIGNATURESEXISTS = 0x1;
    public const ACROFORM_SIG_APPENDONLY = 0x2;
    public const ACROFORM_FIELD_BUTTON = 'Btn';
    public const ACROFORM_FIELD_TEXT = 'Tx';
    public const ACROFORM_FIELD_CHOICE = 'Ch';
    public const ACROFORM_FIELD_SIG = 'Sig';
    public const ACROFORM_FIELD_READONLY = 0x1;
    public const ACROFORM_FIELD_REQUIRED = 0x2;
    public const ACROFORM_FIELD_TEXT_MULTILINE = 0x1000;
    public const ACROFORM_FIELD_TEXT_PASSWORD = 0x2000;
    public const ACROFORM_FIELD_TEXT_RICHTEXT = 0x10000;
    public const ACROFORM_FIELD_CHOICE_COMBO = 0x20000;
    public const ACROFORM_FIELD_CHOICE_EDIT = 0x40000;
    public const ACROFORM_FIELD_CHOICE_SORT = 0x80000;
    public const ACROFORM_FIELD_CHOICE_MULTISELECT = 0x200000;
    public const XOBJECT_SUBTYPE_FORM = 'Form';
    /**
     ...
     */
    protected $indirect_reference_id = 0;
    /**
     * @var integer The objectId (number within the objects array)
     */
    protected $embedded_files_id = 0;
    /**
     * AcroForm objectId
     *
     * @var integer
     ...
     */
    public $default_font = __DIR__ . '/fonts/Helvetica.afm';
    /**
     ...
     */
    protected $compression_ready = false;
    /**
     * @var array Current page size
     */
    protected $current_page_size = ['width' => 0, 'height' => 0];
    /**
     * @var array All the chars that will be required in the font subsets
     */
    protected $string_subsets = [];
    /**
     * @var string The target internal encoding
     */
    protected static $target_encoding = 'Windows-1252';
    /**
     * @var array
     */
     ...
     */
    protected static $core_fonts = ['courier', 'courier-bold', 'courier-oblique', 'courier-boldoblique', 'helvetica', 'helvetica-bold', 'helvetica-oblique', 'helvetica-boldoblique', 'times-roman', 'times-bold', 'times-italic', 'times-bolditalic', 'symbol', 'zapfdingbats'];
    /**
     * Class constructor
     ...
     */
    public function __construct($page_size = [0, 0, 612, 792], $is_unicode = false, $fontcache = '', $tmp = '')
    {
        $this->is_unicode = $is_unicode;
        $this->fontcache = rtrim($fontcache, DIRECTORY_SEPARATOR . '/\\');
        $this->tmp = $tmp !== '' ? $tmp : sys_get_temp_dir();
        $this->new_document($page_size);
        $this->compression_ready = function_exists('gzcompress');
        if (in_array('Windows-1252', mb_list_encodings())) {
            self::$target_encoding = 'Windows-1252';
        }
        // also initialize the font families that are known about already
        $this->set_font_family('init');
    }
    public function __destruct()
    {
        foreach ($this->image_cache as $file) {
            if (file_exists($file)) {
                unlink($file);
            }
        }
     ...
     */
    protected function o_destination($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
     ...
     */
    protected function o_viewer_preferences($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
     ...
     * @param string|array $options
     */
    protected function o_catalog($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            ...
            case 'viewerPreferences':
                if (!isset($o['info']['viewerPreferences'])) {
                    $this->num_obj++;
                    $this->o_viewer_preferences($this->num_obj, 'new');
                    $o['info']['viewerPreferences'] = $this->num_obj;
                }
                $vp = $o['info']['viewerPreferences'];
                $this->o_viewer_preferences($vp, 'add', $options);
                break;
            case 'outputIntents':
                ...
                $this->num_obj++;
                $this->o_contents($this->num_obj, 'new');
                $this->objects[$this->num_obj]['c'] = $options['iccProfileData'];
                $this->o_contents($this->num_obj, 'add', ['N' => $options['colorComponentsCount']]);
                $o['info']['outputIntents'][] = ['iccProfileName' => $options['iccProfileName'], 'destOutputProfile' => $this->num_obj];
                break;
            ...
            case 'metadata':
                $this->num_obj++;
                $o['info']['metadata'] = $this->num_obj;
                $this->o_contents($this->num_obj, 'new');
                $this->objects[$this->num_obj]['c'] = $options;
                $this->o_contents($this->num_obj, 'add', ['Type' => '/Metadata', 'Subtype' => '/XML']);
                break;
            case 'out':
                $res = "\n{$id} 0 obj\n<< /Type /Catalog";
                foreach ($o['info'] as $k => $v) {
     ...
     * @param string $options
     */
    protected function o_pages($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $this->o_catalog($this->catalog_id, 'pages', $id);
                break;
            case 'page':
                if (!is_array($options)) {
                    // then it will just be the id of the new page
                    $o['info']['pages'][] = $options;
                } else if (isset($options['id']) && isset($options['rid']) && isset($options['pos'])) {
            ...
            case 'mediaBox':
                ...
                $this->current_page_size = ['width' => $options[2], 'height' => $options[3]];
                break;
            case 'font':
     ...
     * @param string $options
     */
    protected function o_outlines($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $this->o_catalog($this->catalog_id, 'outlines', $id);
                break;
            case 'outline':
     ...
     */
    protected function o_font($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $font_num = $this->num_fonts;
                $this->objects[$id]['info']['fontNum'] = $font_num;
                // deal with the encoding and the differences
                if (isset($options['differences'])) {
                    // then we'll need an encoding dictionary
                    $this->num_obj++;
                    $this->o_font_encoding($this->num_obj, 'new', $options);
                    $this->objects[$id]['info']['encodingDictionary'] = $this->num_obj;
                ...
                if ($this->fonts[$options['fontFileName']]['isUnicode']) {
                    ...
                    $to_unicode_id = ++$this->num_obj;
                    $this->o_to_unicode($to_unicode_id, 'new');
                    $this->objects[$id]['info']['toUnicode'] = $to_unicode_id;
                    $cid_font_id = ++$this->num_obj;
                    $this->o_font_descendent_cid($cid_font_id, 'new', $options);
                    $this->objects[$id]['info']['cidFont'] = $cid_font_id;
                }
                // also tell the pages node about the new font
                $this->o_pages($this->current_node, 'font', ['fontNum' => $font_num, 'objNum' => $id]);
                break;
            case 'add':
                ...
                if ($font_options !== false) {
                    ...
                    if (isset($o['info']['cidFont'])) {
                        $this->o_font_descendent_cid($o['info']['cidFont'], 'add', $font_options);
                    }
                }
                break;
            case 'out':
    ...
    protected function get_font_subsetting_tag(array $font): string
    {
     ...
     * @throws Exception
     */
    private function process_font(int $font_obj_id, array $object_info)
    {
        $font_file_name = $object_info['fontFileName'];
        if (!isset($this->fonts[$font_file_name])) {
            return false;
        }
        $font =& $this->fonts[$font_file_name];
        $file_suffix = $font['fileSuffix'];
        $file_suffix_lower = strtolower($font['fileSuffix']);
        $fbfile = "{$font_file_name}.{$file_suffix}";
        $is_ttf_font = $file_suffix_lower === 'ttf';
        $is_pfb_font = $file_suffix_lower === 'pfb';
        $this->add_message('selectFont: checking for - ' . $fbfile);
        ...
        if (!$file_suffix) {
            $this->add_message('selectFont: pfb or ttf file not found, ok if this is one of the 14 standard fonts');
            return false;
        }
        $adobe_font_name = $font['PostScriptName'] ?? $font['FontName'];
        //        $fontObj = $this->numObj;
        $this->add_message("selectFont: adding font file - {$fbfile} - {$adobe_font_name}");
        ...
        foreach ($font['C'] as $num => $d) {
            if (intval($num) > 0 || $num == '0') {
                if (!$font['isUnicode']) {
                    ...
                    if ($last_char > 0 && $num > $last_char + 1) {
                        for ($i = $last_char + 1; $i < $num; $i++) {
                            $widths[] = 0;
                        }
                    }
                }
        ...
        if (isset($object_info['differences'])) {
            foreach ($object_info['differences'] as $char_num => $char_name) {
                if ($char_num > $last_char) {
                    if (!$object_info['isUnicode']) {
                        // With Unicode, widths array isn't used
                        for ($i = $last_char + 1; $i <= $char_num; $i++) {
                            $widths[] = 0;
                        }
                    }
                    $last_char = $char_num;
                }
                if (isset($font['C'][$char_name])) {
                    $widths[$char_num - $first_char] = $font['C'][$char_name];
                    if ($font['isUnicode']) {
                        $cid_widths[$char_name] = $font['C'][$char_name];
                    }
                }
            }
        }
        ...
        $this->add_message('selectFont: FirstChar = ' . $first_char);
        $this->add_message('selectFont: LastChar = ' . $last_char);
        $widthid = -1;
        if (!$font['isUnicode']) {
            // With Unicode, widths array isn't used
            $this->num_obj++;
            $this->o_contents($this->num_obj, 'new', 'raw');
            $this->objects[$this->num_obj]['c'] .= '[' . implode(' ', $widths) . ']';
            $widthid = $this->num_obj;
        }
        $missing_width = 500;
        $stem_v = 70;
        ...
        if (!$font['isSubsetting']) {
            ...
            $adobe_font_name = $this->get_font_subsetting_tag($font) . '+' . $adobe_font_name;
            $this->string_subsets[$font_file_name][] = 32;
            // Force space if not in yet
            $subset = $this->string_subsets[$font_file_name];
            sort($subset);
            // Load font
            $font_obj = Font::load($fbfile);
            $font_obj->parse();
            // Define subset
            $font_obj->set_subset($subset);
            $font_obj->reduce();
            // Write new font
            $tmp_name = @tempnam($this->tmp, 'cpdf_subset_');
            $font_obj->open($tmp_name, Binary_Stream::modeReadWrite);
            ...
            $subtable = null;
            foreach ($font_obj->get_data('cmap', 'subtables') as $_subtable) {
            ...
            if ($subtable) {
                $glyph_index_array = $subtable['glyphIndexArray'];
                $hmtx = $font_obj->get_data('hmtx');
                unset($glyph_index_array[0xffff]);
                $cidtogid = str_pad('', max(array_keys($glyph_index_array)) * 2 + 1, "\x00");
                $font['CIDWidths'] = [];
                foreach ($glyph_index_array as $cid => $gid) {
                    if ($cid >= 0 && $cid < 0xffff && $gid) {
                        $cidtogid[$cid * 2] = chr($gid >> 8);
                        $cidtogid[$cid * 2 + 1] = chr($gid & 0xff);
                    }
                    $width = $font_obj->normalize_f_unit(isset($hmtx[$gid]) ? $hmtx[$gid][0] : $hmtx[0][0]);
        ...
        $this->num_obj++;
        $font_descriptor_id = $this->num_obj;
        $this->num_obj++;
        $pfbid = $this->num_obj;
        ...
        $flags += 2 ** 5;
        // assume non-sybolic
        $list = ['Ascent' => 'Ascender', 'CapHeight' => 'CapHeight', 'MissingWidth' => 'MissingWidth', 'Descent' => 'Descender', 'FontBBox' => 'FontBBox', 'ItalicAngle' => 'ItalicAngle'];
        $fdopt = ['Flags' => $flags, 'FontName' => $adobe_font_name, 'StemV' => $stem_v];
        foreach ($list as $k => $v) {
            if (isset($font[$v])) {
                $fdopt[$k] = $font[$v];
            }
        ...
        $this->o_font_descriptor($font_descriptor_id, 'new', $fdopt);
        ...
        if ($is_pfb_font) {
            ...
            $this->o_contents($this->num_obj, 'add', ['Length1' => $l1, 'Length2' => $l2, 'Length3' => $l3]);
        } elseif ($is_ttf_font) {
            $l1 = mb_strlen($data, '8bit');
            $this->o_contents($this->num_obj, 'add', ['Length1' => $l1]);
        }
        // tell the font object about all this new stuff
        $options = ['BaseFont' => $adobe_font_name, 'MissingWidth' => $missing_width, 'Widths' => $widthid, 'FirstChar' => $first_char, 'LastChar' => $last_char, 'FontDescriptor' => $font_descriptor_id];
        if ($is_ttf_font) {
            $options['SubType'] = 'TrueType';
        }
        $this->add_message("adding extra info to font.({$font_obj_id})");
        foreach ($options as $fk => $fv) {
            $this->add_message("{$fk} : {$fv}");
        }
        return $options;
    }
    /**
     ...
     * @param $action
     */
    protected function o_to_unicode($id, $action): ?string
    {
        switch ($action) {
            case 'new':
            ...
            case 'out':
                ...
                $registry = 'Adobe';
                if ($this->encrypted) {
                    $this->encrypt_init($id);
                    $ordering = $this->filter_text($this->ARC4($ordering), false, false);
                    $registry = $this->filter_text($this->ARC4($registry), false, false);
                }
                $stream = <<<EOT
     ...
     * @param string $options
     */
    protected function o_font_descriptor($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
     ...
     * @param string $options
     */
    protected function o_font_encoding($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
     ...
     * @param string|array $options
     */
    protected function o_font_descendent_cid($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $cid_system_info_id = ++$this->num_obj;
                $this->o_cid_system_info($cid_system_info_id, 'new');
                $this->objects[$id]['info']['cidSystemInfo'] = $cid_system_info_id;
                // and a CID to GID map
                $cid_to_gid_map_id = ++$this->num_obj;
                $this->o_font_gi_dto_cid_map($cid_to_gid_map_id, 'new', $options);
                $this->objects[$id]['info']['cidToGidMap'] = $cid_to_gid_map_id;
                break;
            case 'add':
                foreach ($options as $k => $v) {
                    switch ($k) {
                        ...
                        case 'FontDescriptor':
                        case 'SubType':
                            $this->add_message("o_fontDescendentCID {$k} : {$v}");
                            $o['info'][$k] = $v;
                            break;
                    }
                }
     ...
     * @param $action
     */
    protected function o_cid_system_info($id, $action): ?string
    {
        switch ($action) {
            case 'new':
     ...
     * @param string $options
     */
    protected function o_font_gi_dto_cid_map($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            ...
            case 'out':
                $res = "\n{$id} 0 obj\n";
                $font_file_name = $o['info']['fontFileName'];
                $tmp = $this->fonts[$font_file_name]['CIDtoGID'] = base64_decode($this->fonts[$font_file_name]['CIDtoGID']);
                $compressed = isset($this->fonts[$font_file_name]['CIDtoGID_Compressed']) && $this->fonts[$font_file_name]['CIDtoGID_Compressed'];
                if (!$compressed && isset($o['raw'])) {
                    $res .= $tmp;
                } else {
                    $res .= '<<';
                    if (!$compressed && $this->compression_ready && $this->options['compression']) {
     ...
     * @param string $options
     */
    protected function o_procset($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $this->o_pages($this->current_node, 'procset', $id);
                $this->procset_object_id = $id;
                break;
            case 'add':
     ...
     * @param string $options
     */
    protected function o_info($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
                $this->info_object = $id;
            ...
            case 'out':
                ...
                foreach ($o['info'] as $k => $v) {
                    ...
                    if ($k !== 'CreationDate' && $k !== 'ModDate') {
                        $v = $this->utf8to_utf16be($v);
                    }
                    if ($encrypted) {
                        $v = $this->ARC4($v);
                    }
                    $res .= $this->filter_text($v, false, false);
                    $res .= ")\n";
                }
                return $res . ">>\nendobj";
            case 'Title':
     ...
     * @param string $options
     */
    protected function o_action($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
            ...
            case 'out':
                ...
                switch ($o['type']) {
                    ...
                    case 'URI':
                        $res .= "\n/S /URI\n/URI (";
                        if ($this->encrypted) {
                            $res .= $this->filter_text($this->ARC4($o['info']), false, false);
                        } else {
                            $res .= $this->filter_text($o['info'], false, false);
                        }
                        $res .= ')';
                        break;
                }
     ...
     * @param string $options
     */
    protected function o_annotation($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                ...
                $this->o_page($page_id, 'annot', $id);
                // and add the action object which is going to be required
                switch ($options['type']) {
                    case 'link':
                        $this->objects[$id] = ['t' => 'annotation', 'info' => $options];
                        $this->num_obj++;
                        $this->o_action($this->num_obj, 'new', $options['url']);
                        $this->objects[$id]['info']['actionId'] = $this->num_obj;
                        break;
                    case 'ilink':
                        ...
                        $this->objects[$id] = ['t' => 'annotation', 'info' => $options];
                        $this->num_obj++;
                        $this->o_action($this->num_obj, 'new', ['type' => 'ilink', 'label' => $label]);
                        $this->objects[$id]['info']['actionId'] = $this->num_obj;
                        break;
                }
                break;
     ...
     * @param string $options
     */
    protected function o_page($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            case 'new':
                $this->num_pages++;
                $this->objects[$id] = ['t' => 'page', 'info' => ['parent' => $this->current_node, 'pageNum' => $this->num_pages, 'mediaBox' => $this->objects[$this->current_node]['info']['mediaBox']]];
                if (is_array($options)) {
                    // then this must be a page insertion, array should contain 'rid','pos'=[before|after]
                    $options['id'] = $id;
                    $this->o_pages($this->current_node, 'page', $options);
                } else {
                    $this->o_pages($this->current_node, 'page', $id);
                }
                $this->current_page = $id;
                //make a contents object to go with this page
                $this->num_obj++;
                $this->o_contents($this->num_obj, 'new', $id);
                $this->current_contents = $this->num_obj;
                $this->objects[$id]['info']['contents'] = [];
                $this->objects[$id]['info']['contents'][] = $this->num_obj;
                $match = $this->num_pages % 2 ? 'odd' : 'even';
                foreach ($this->add_loose_objects as $o_id => $target) {
                    if ($target === 'all' || $match === $target) {
                        $this->objects[$id]['info']['contents'][] = $o_id;
                    }
                }
                break;
            case 'content':
            ...
            case 'out':
                ...
                if ($count == 1) {
                    ...
                    foreach ($o['info']['contents'] as $c_id) {
                        $res .= "{$c_id} 0 R\n";
                    }
                    $res .= ']';
                }
                ...
                if ($this->pdfa) {
                    $pages_info = $this->objects[$this->current_node]['info'];
                    if (isset($pages_info['fonts']) && count($pages_info['fonts']) || isset($pages_info['procset']) || isset($pages_info['extGStates']) && count($pages_info['extGStates'])) {
                        $res .= "\n/Resources <<";
                        if (isset($pages_info['procset'])) {
                            $res .= "\n/ProcSet " . $pages_info['procset'] . ' 0 R';
                        }
                        if (isset($pages_info['fonts']) && count($pages_info['fonts'])) {
                            $res .= "\n/Font << ";
                            foreach ($pages_info['fonts'] as $finfo) {
                                $res .= "\n/F" . $finfo['fontNum'] . ' ' . $finfo['objNum'] . ' 0 R';
                            }
                            $res .= "\n>>";
                        }
                        if (isset($pages_info['xObjects']) && count($pages_info['xObjects'])) {
                            $res .= "\n/XObject << ";
                            foreach ($pages_info['xObjects'] as $finfo) {
                                $res .= "\n/" . $finfo['label'] . ' ' . $finfo['objNum'] . ' 0 R';
                            }
                            $res .= "\n>>";
                        }
                        if (isset($pages_info['extGStates']) && count($pages_info['extGStates'])) {
                            $res .= "\n/ExtGState << ";
                            foreach ($pages_info['extGStates'] as $gstate) {
                                $res .= "\n/GS" . $gstate['stateNum'] . ' ' . $gstate['objNum'] . ' 0 R';
                            }
                            $res .= "\n>>";
                        }
     ...
     * @param string|array $options
     */
    protected function o_contents($id, $action, $options = ''): ?string
    {
        if ($action !== 'new') {
            $o =& $this->objects[$id];
        }
        switch ($action) {
            ...
            case 'out':
                ...
                if (isset($this->objects[$id]['raw'])) {
                    ...
                    $res .= '<<';
                    if ($this->compression_ready && $this->options['compression']) {
                        // then implement ZLIB based compression on this content stream
     ...
     * @param $action
     */
    protected function o_embedjs($id, $action): ?string
    {
        switch ($action) {
            case 'new':
     ...
     * @param string $code
     */
    protected function o_javascript($id, $action, $code = ''): ?string
    {
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'javascript', 'info' => ['S' => '/JavaScript', 'JS' => '(' . $this->filter_text($code, true, false) . ')']];
                break;
            case 'out':
                $o =& $this->objects[$id];
                $res = "\n{$id} 0 obj\n<< ";
     ...
     * @throws Exception
     */
    protected function o_image($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
                // make the new object
                ...
                if (!isset($options['type']) || $options['type'] === 'jpg') {
                    ...
                    if ($options['isMask']) {
                        $info['ColorSpace'] = '/DeviceGray';
                    } else if (mb_strlen($options['pdata'], '8bit')) {
                        $tmp = ' [ /Indexed /DeviceRGB ' . (mb_strlen($options['pdata'], '8bit') / 3 - 1) . ' ';
                        $this->num_obj++;
                        $this->o_contents($this->num_obj, 'new');
                        $this->objects[$this->num_obj]['c'] = $options['pdata'];
                        $tmp .= $this->num_obj . ' 0 R';
                        $tmp .= ' ]';
                        $info['ColorSpace'] = $tmp;
                ...
                $this->o_pages($this->current_node, 'xObject', ['label' => $options['label'], 'objNum' => $id]);
                // also make sure that we have the right procset object for it.
                $this->o_procset($this->procset_object_id, 'add', 'ImageC');
                break;
            case 'out':
                $o =& $this->objects[$id];
                $tmp =& $o['data'];
     ...
     * @param string $options
     */
    protected function o_ext_g_state($id, $action, $options = ''): ?string
    {
        ...
        switch ($action) {
            case 'new':
                ...
                $this->num_states++;
                $this->o_pages($this->current_node, 'extGState', ['objNum' => $id, 'stateNum' => $this->num_states]);
                break;
            case 'out':
                $o =& $this->objects[$id];
                $res = "\n{$id} 0 obj\n<< /Type /ExtGState\n";
     ...
     * @return string
     */
    protected function o_xobject($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
     ...
     * @param string $options
     */
    protected function o_acroform($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
                $this->o_catalog($this->catalog_id, 'acroform', $id);
     ...
     * @param mixed $options
     */
    protected function o_field($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
                $this->o_page($options['pageid'], 'annot', $id);
                $this->o_acroform($this->acro_form_id, 'addfield', $id);
            ...
            case 'out':
                ...
                foreach ($o['info'] as $k => $v) {
                    switch ($k) {
                        ...
                        case 'value':
                            if ($encrypted) {
                                $v = $this->filter_text($this->ARC4($v), false, false);
                            }
                            $res .= "/V ({$v})\n";
                            break;
                        case 'refvalue':
                        ...
                        case 'options':
                            ...
                            foreach ($v as $opt) {
                                if ($encrypted) {
                                    $opt = $this->filter_text($this->ARC4($opt), false, false);
                                }
                                $res .= "({$opt})\n";
                            }
                            $res .= "]\n";
     ...
     * @param string $options
     */
    protected function o_sig($id, $action, $options = ''): ?string
    {
        $sign_maxlen = $this->signature_max_len;
        switch ($action) {
            case 'new':
                $this->objects[$id] = ['t' => 'sig', 'info' => $options];
                $this->byte_range[$id] = ['t' => 'sig'];
                break;
            case 'byterange':
                $o =& $this->objects[$id];
                ...
                $range_start_pos = $pos + $len + 1 + 10;
                // before '<'
                $content = substr_replace($content, str_pad(sprintf('/ByteRange [ 0 %u %u %u ]', $range_start_pos, $range_start_pos + $sign_maxlen + 2, $content_len - 2 - $sign_maxlen - $range_start_pos), $len, ' ', STR_PAD_RIGHT), $pos, $len);
                $fuid = uniqid();
                $tmp_input = $this->tmp . '/pkcs7.tmp.' . $fuid . '.in';
                $tmp_output = $this->tmp . '/pkcs7.tmp.' . $fuid . '.out';
                if (file_put_contents($tmp_input, substr($content, 0, $range_start_pos)) === false) {
                    throw new \Exception('Unable to write temporary file for signing.');
                }
                if (file_put_contents($tmp_input, substr($content, $range_start_pos + 2 + $sign_maxlen), FILE_APPEND) === false) {
                    throw new \Exception('Unable to write temporary file for signing.');
                }
                if (openssl_pkcs7_sign($tmp_input, $tmp_output, $o['info']['SignCert'], [$o['info']['PrivKey'], $o['info']['Password']], [], PKCS7_BINARY | PKCS7_DETACHED) === false) {
                    throw new \Exception('Failed to prepare signature.');
                }
                $signature = file_get_contents($tmp_output);
                unlink($tmp_input);
                unlink($tmp_output);
                $sign = substr($signature, strpos($signature, "%%EOF\n\n------") + 13);
                [$head, $signature] = explode("\n\n", $sign);
                ...
                $content = substr_replace($content, $signature, $range_start_pos + 1, $sign_maxlen);
                break;
            case 'out':
                $res = "\n{$id} 0 obj\n<<\n";
                $encrypted = $this->encrypted;
                ...
                if ($encrypted) {
                    $date = $this->filter_text($this->ARC4($date), false, false);
                }
                $res .= "/M ({$date})\n";
                ...
                foreach ($o['info'] as $k => $v) {
                    switch ($k) {
                        ...
                        case 'ContactInfo':
                            if ($v !== null && $v !== '') {
                                $res .= "/{$k} (" . ($encrypted ? $this->filter_text($this->ARC4($v), false, false) : $v) . ") \n";
                            }
                            break;
                    }
                }
     ...
     * @param string $options
     */
    protected function o_encryption($id, $action, $options = ''): ?string
    {
        switch ($action) {
            case 'new':
                // make the new object
            ...
            case 'keys':
                ...
                $tmp = $this->md5_16($user . $ovalue . chr($info['p']) . chr(255) . chr(255) . chr(255) . hex2bin($this->file_identifier));
                $ukey = substr($tmp, 0, 5);
                $this->ARC4_init($ukey);
                $this->encryption_key = $ukey;
                $this->encrypted = true;
            ...
            case 'out':
                ...
                $res .= "\n/V 1";
                $res .= "\n/R 2";
                $res .= "\n/O (" . $this->filter_text($o['info']['O'], false, false) . ')';
                $res .= "\n/U (" . $this->filter_text($o['info']['U'], false, false) . ')';
    ...
    protected function o_indirect_references($id, $action, $options = null): ?string
    {
        switch ($action) {
            case 'new':
            case 'add':
                if ($id === 0) {
                    $id = ++$this->num_obj;
                    $this->o_catalog($this->catalog_id, 'names', $id);
                    $this->objects[$id] = ['t' => 'indirect_references', 'info' => $options];
                    $this->indirect_reference_id = $id;
                } else {
                    $this->objects[$id]['info'] = array_merge($this->objects[$id]['info'], $options);
                }
                break;
            case 'out':
                $res = "\n{$id} 0 obj\n<< ";
                foreach ($this->objects[$id]['info'] as $reference_obj_name => $reference_obj_id) {
                    $res .= "/{$reference_obj_name} {$reference_obj_id} 0 R ";
                }
                return $res . ">>\nendobj";
        }
        return null;
    }
    protected function o_names($id, $action, $options = null): ?string
    {
        switch ($action) {
            case 'new':
            case 'add':
                if ($id === 0) {
                    ...
                    $this->o_indirect_references($this->indirect_reference_id, 'add', ['EmbeddedFiles' => $id]);
                    $this->embedded_files_id = $id;
                } else {
            ...
            case 'out':
                ...
                if (count($info) > 0) {
                    ...
                    foreach ($info as $entry) {
                        ...
                        $filename = $this->filter_text($filename, false, false);
    ...
    protected function o_embedded_file_dictionary($id, $action, $options = null): ?string
    {
        switch ($action) {
            case 'new':
                $embedded_file_id = ++$this->num_obj;
                $options['embedded_reference'] = $embedded_file_id;
                $this->objects[$id] = ['t' => 'embedded_file_dictionary', 'info' => $options];
                $this->o_embedded_file($embedded_file_id, 'new', $options);
                $options['dict_reference'] = $id;
                $this->o_names($this->embedded_files_id, 'add', $options);
                break;
            case 'afRelationship':
                $info =& $this->objects[$id]['info'];
            ...
            case 'out':
                $info =& $this->objects[$id]['info'];
                $filename = $this->utf8to_utf16be($info['filename']);
                $description = $this->utf8to_utf16be($info['description']);
                if ($this->encrypted) {
                    $this->encrypt_init($id);
                ...
                $filename = $this->filter_text($filename, false, false);
                $description = $this->filter_text($description, false, false);
                $res = "\n{$id} 0 obj\n<</Type /Filespec";
                $res .= "\n /F ({$filename}) /UF ({$filename}) /Desc ({$description})";
                if (isset($info['afRelationship'])) {
                    $res .= "\n /AFRelationship /{$this->filter_name($info['afRelationship'])}";
                }
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                if ($this->compression_ready && $this->options['compression']) {
                    ...
                    $file_content = '';
                    $deflate_context = deflate_init(ZLIB_ENCODING_DEFLATE, ['level' => 6]);
                    foreach ($blocks as $block) {
                        $file_content .= deflate_add($deflate_context, $block, ZLIB_NO_FLUSH);
                    }
                    $file_content .= deflate_add($deflate_context, '', ZLIB_FINISH);
                ...
                if ($this->encrypted) {
                    ...
                    $file_content_size = mb_strlen($file_content, '8bit');
                    $checksum = $this->filter_text($this->ARC4($checksum), false, false);
                    $creation = $this->filter_text($this->ARC4($creation), false, false);
                    $modified = $this->filter_text($this->ARC4($modified), false, false);
                }
                $res = "\n{$id} 0 obj\n<</Type /EmbeddedFile";
                if (!empty($info['mimeType'])) {
                    $res .= ' /Subtype /' . $this->filter_name($info['mimeType']);
                }
                if ($this->compression_ready && $this->options['compression']) {
                    $res .= ' /Filter/FlateDecode';
                }
                return $res . (" /Length {$file_content_size}" . "\n /Params <</Size {$file_size} /CheckSum ({$checksum}) /CreationDate ({$created}) /ModDate ({$modified}) >>" . " >>\nstream\n{$file_content}\nendstream\nendobj");
        }
        return null;
    }
    /**
     ...
     */
    public function enable_pdf_a_compliance(): void
    {
        $this->pdfa = true;
        $icc_profile_path = __DIR__ . '/res/sRGB2014.icc';
        $this->o_catalog($this->catalog_id, 'outputIntents', ['iccProfileData' => file_get_contents($icc_profile_path), 'iccProfileName' => basename($icc_profile_path), 'colorComponentsCount' => '3']);
    }
    public function set_additional_xmp_rdf(string $xml_rdf_contents): void
    {
        $this->additional_xmp_rdf = $xml_rdf_contents;
    }
    /**
     * Generate the Metadata XMP XML for PDF/A
     */
    public function get_xmp_metadata(): string
    {
        $md = <<<EOT
        ...
        EOT;
        $info = $this->objects[$this->info_object]['info'];
        if (isset($info['Title'])) {
        ...
        if (isset($info['CreationDate']) && $date = $this->parse_pdf_date($info['CreationDate'])) {
            $md .= "\n<xmp:CreateDate>";
            $md .= $date->format("Y-m-d\\TH:i:sP");
            $md .= '</xmp:CreateDate>';
        }
        if (isset($info['ModDate']) && $date = $this->parse_pdf_date($info['ModDate'])) {
            $md .= "\n<xmp:ModifyDate>";
            $md .= $date->format("Y-m-d\\TH:i:sP");
            $md .= '</xmp:ModifyDate>';
        }
        $md .= "\n</rdf:Description>";
        $md .= $this->additional_xmp_rdf;
        return $md . "\n</rdf:RDF>\n</x:xmpmeta>\n<?xpacket end=\"w\"?>";
    }
    /**
     ...
     */
    public function parse_pdf_date($date)
    {
        $formats = ['Y', 'Ym', 'Ymd', 'YmdH', 'YmdHi', 'YmdHis', 'YmdHisO'];
        $date = substr($date, 2);
        ...
        foreach ($formats as $format) {
            $parsed_date = \DateTime::create_from_format($format, $date, new \DateTimeZone('UTC'));
            if ($parsed_date) {
                return $parsed_date;
            }
        }
     ...
     * @param $id
     */
    public function encrypt_init($id): void
    {
        $tmp = $this->encryption_key;
        $hex = dechex($id);
     ...
     */
    public function ARC4_init(string $key = ''): void
    {
        $this->arc4 = '';
     ...
     * @param $y1
     */
    public function add_link($url, $x0, $y0, $x1, $y1): void
    {
        $this->num_obj++;
        $info = ['type' => 'link', 'url' => $url, 'rect' => [$x0, $y0, $x1, $y1]];
        $this->o_annotation($this->num_obj, 'new', $info);
    }
    /**
     ...
     * @param $y1
     */
    public function add_internal_link($label, $x0, $y0, $x1, $y1): void
    {
        $this->num_obj++;
     ...
     * @param array $pc
     */
    public function set_encryption($user_pass = '', $owner_pass = '', $pc = []): void
    {
        $p = bindec('11000000');
        ...
        if ($this->arc4_objnum == 0) {
            ...
            $this->num_obj++;
            if (!isset($owner_pass) || mb_strlen($owner_pass) === 0) {
                $owner_pass = $user_pass;
            }
            $this->o_encryption($this->num_obj, 'new', ['user' => $user_pass, 'owner' => $owner_pass, 'p' => $p]);
        }
    }
    /**
     * should be used for internal checks, not implemented as yet
     */
    public function check_all_here()
    {
    }
    /**
     ...
     * @param bool $debug
     */
    public function output($debug = false): string
    {
        if ($debug) {
        ...
        if ($this->javascript) {
            ...
            $this->o_javascript(++$this->num_obj, 'new', $this->javascript);
            $id = $this->catalog_id;
            $this->o_indirect_references($this->indirect_reference_id, 'add', ['JavaScript' => $js_id]);
        }
        if ($this->pdfa) {
            $this->o_catalog($this->catalog_id, 'metadata', $this->get_xmp_metadata());
        }
        if ($this->file_identifier === '') {
            $tmp = implode('', $this->objects[$this->info_object]['info']);
            $this->file_identifier = md5('DOMPDF' . __FILE__ . $tmp . microtime() . mt_rand());
        }
        if ($this->arc4_objnum) {
            $this->o_encryption($this->arc4_objnum, 'keys');
            $this->ARC4_init($this->encryption_key);
        }
        $this->check_all_here();
        $xref = [];
        ...
        $content .= "trailer\n<<\n" . '/Size ' . (count($xref) + 1) . "\n" . '/Root 1 0 R' . "\n" . '/Info ' . $this->info_object . " 0 R\n";
        // if encryption has been applied to this document then add the marker for this dictionary
        if ($this->arc4_objnum > 0) {
            $content .= '/Encrypt ' . $this->arc4_objnum . " 0 R\n";
        }
        $content .= '/ID[<' . $this->file_identifier . '><' . $this->file_identifier . ">]\n";
        // account for \n added at start of xref table
        $pos++;
        $content .= ">>\nstartxref\n{$pos}\n%%EOF\n";
        foreach ($this->byte_range as $k => $v) {
            $tmp = 'o_' . $v['t'];
     ...
     */
    private function new_document($page_size = [0, 0, 612, 792]): void
    {
        $this->num_obj = 0;
        $this->objects = [];
        $this->num_obj++;
        $this->o_catalog($this->num_obj, 'new');
        $this->num_obj++;
        $this->o_outlines($this->num_obj, 'new');
        $this->num_obj++;
        $this->o_pages($this->num_obj, 'new');
        $this->o_pages($this->num_obj, 'mediaBox', $page_size);
        $this->current_node = 3;
        $this->num_obj++;
        $this->o_procset($this->num_obj, 'new');
        $this->num_obj++;
        $this->o_info($this->num_obj, 'new');
        $this->num_obj++;
        $this->o_page($this->num_obj, 'new');
        // need to store the first page id as there is no way to get it to the user during
        // startup
        $this->first_page_id = $this->current_contents;
    }
    /**
     ...
     * @param $font
     */
    private function open_font(string $font): void
    {
        ...
        $this->add_message("openFont: {$font} - {$name}");
        if (!$this->is_unicode || in_array(mb_strtolower(basename($name), 'UTF-8'), self::$core_fonts)) {
            $metrics_name = "{$name}.afm";
        } else {
            $metrics_name = "{$name}.ufm";
        }
        $cache_name = "{$metrics_name}.json";
        $this->add_message("metrics: {$metrics_name}, cache: {$cache_name}");
        if (file_exists($fontcache . '/' . $cache_name)) {
            $this->add_message("openFont: json metrics file exists {$fontcache}/{$cache_name}");
            $cached_font_info = json_decode(file_get_contents($fontcache . '/' . $cache_name), true);
            if (!isset($cached_font_info['_version_']) || $cached_font_info['_version_'] != $this->fontcache_version) {
                $this->add_message('openFont: font cache is out of date, regenerating');
            } else {
        ...
        if (!isset($this->fonts[$font]) && file_exists("{$dir}/{$metrics_name}")) {
            ...
            $this->add_message("openFont: build php file from {$dir}/{$metrics_name}");
            $data = [];
            // 20 => 'space'
            $data['codeToName'] = [];
            ...
            $data['_version_'] = $this->fontcache_version;
        ...
        if (!isset($this->fonts[$font])) {
            $this->add_message("openFont: no font file found for {$font}.");
        }
    }
    /**
     ...
     * @throws FontNotFoundException
     */
    public function select_font($font_name, $encoding = '', $set = true, $is_subsetting = true)
    {
        $font_name = (string) $font_name;
        $ext = substr($font_name, -4);
        if ($ext === '.afm' || $ext === '.ufm') {
            $font_name = mb_substr($font_name, 0, mb_strlen($font_name, 'UTF-8') - 4, 'UTF-8');
        }
        if ($font_name === '') {
            return $this->current_font_num;
        }
        if (!isset($this->fonts[$font_name])) {
            $this->add_message("selectFont: selecting - {$font_name} - {$encoding}, {$set}");
            // load the file
            $this->open_font($font_name);
            if (isset($this->fonts[$font_name])) {
                $this->num_obj++;
                $this->num_fonts++;
                $font =& $this->fonts[$font_name];
                $name = basename($font_name);
                $options = ['name' => $name, 'fontFileName' => $font_name, 'isSubsetting' => $is_subsetting];
                ...
                $this->o_font($this->num_obj, 'new', $options);
                if (file_exists("{$font_name}.ttf")) {
                    $file_suffix = 'ttf';
                } elseif (file_exists("{$font_name}.TTF")) {
                    $file_suffix = 'TTF';
                } elseif (file_exists("{$font_name}.pfb")) {
                    $file_suffix = 'pfb';
                } elseif (file_exists("{$font_name}.PFB")) {
                    $file_suffix = 'PFB';
                } else {
                ...
                $font['fileSuffix'] = $file_suffix;
                $font['fontNum'] = $this->num_fonts;
                $font['isSubsetting'] = $is_subsetting && $font['isUnicode'] && strtolower($file_suffix) === 'ttf';
        ...
        if ($set && isset($this->fonts[$font_name])) {
            // so if for some reason the font was not set in the last one then it will not be selected
            $this->current_base_font = $font_name;
            // the next lines mean that if a new font is selected, then the current text state will be
            // applied to it as well.
            $this->current_font = $this->current_base_font;
            $this->current_font_num = $this->fonts[$this->current_font]['fontNum'];
        }
        return $this->current_font_num;
    }
    /**
     ...
     * currentBaseFont.
     */
    private function set_current_font(): void
    {
    ...
    {
        $this->objects[$this->current_contents]['c'] .= $content;
    }
    /**
     ...
     * @throws Exception
     */
    public function set_color(array $color, $force = false): void
    {
        $new_color = [$color[0], $color[1], $color[2], $color[3] ?? null];
        if (!$force && $this->current_color == $new_color) {
            return;
        }
        if (isset($new_color[3])) {
            ...
            $this->add_content(vsprintf("\n%.3F %.3F %.3F %.3F k", $this->current_color));
        } else if (isset($new_color[2])) {
            $this->current_color = $new_color;
            $this->add_content(vsprintf("\n%.3F %.3F %.3F rg", $this->current_color));
        }
    }
    /**
     * sets the color for fill operations
     ...
     */
    public function set_fill_rule($fill_rule): void
    {
        if (!in_array($fill_rule, ['nonzero', 'evenodd'])) {
            return;
        }
     ...
     * @throws Exception
     */
    public function set_stroke_color(array $color, $force = false): void
    {
        $new_color = [$color[0], $color[1], $color[2], $color[3] ?? null];
        if (!$force && $this->current_stroke_color == $new_color) {
            return;
        }
        if (isset($new_color[3])) {
            if ($this->pdfa) {
            ...
            $this->current_stroke_color = $new_color;
            $this->add_content(vsprintf("\n%.3F %.3F %.3F %.3F K", $this->current_stroke_color));
        } else if (isset($new_color[2])) {
            $this->current_stroke_color = $new_color;
            $this->add_content(vsprintf("\n%.3F %.3F %.3F RG", $this->current_stroke_color));
        }
    }
    /**
     ...
     * @param $parameters
     */
    public function set_graphics_state($parameters): void
    {
        // Create a new graphics state object if necessary
        if (($gstate = array_search($parameters, $this->gstates)) === false) {
            $this->num_obj++;
            $this->o_ext_g_state($this->num_obj, 'new', $parameters);
            $gstate = $this->num_states;
     ...
     */
    public function set_line_transparency(string $mode, float $opacity): void
    {
        ...
        if (!in_array($mode, $blend_modes, true)) {
            $mode = 'Normal';
        }
        $new_state = ['mode' => $mode, 'opacity' => $opacity];
        if ($new_state === $this->current_line_transparency) {
            return;
        }
        $this->current_line_transparency = $new_state;
        $options = ['BM' => "/{$mode}", 'CA' => $opacity];
        $this->set_graphics_state($options);
    }
    /**
     ...
     * @param float  $opacity 0.0 fully transparent, 1.0 fully opaque
     */
    public function set_fill_transparency(string $mode, float $opacity): void
    {
        static $blend_modes = ['Normal', 'Multiply', 'Screen', 'Overlay', 'Darken', 'Lighten', 'ColorDogde', 'ColorBurn', 'HardLight', 'SoftLight', 'Difference', 'Exclusion'];
        if (!in_array($mode, $blend_modes, true)) {
            $mode = 'Normal';
        ...
        $new_state = ['mode' => $mode, 'opacity' => $opacity];
        if ($new_state === $this->current_fill_transparency) {
            return;
        }
        $this->current_fill_transparency = $new_state;
        $options = ['BM' => "/{$mode}", 'ca' => $opacity];
        $this->set_graphics_state($options);
    }
    /**
     ...
     * @param bool  $stroke
     */
    public function line($x1, $y1, $x2, $y2, $stroke = true): void
    {
        $this->add_content(sprintf("\n%.3F %.3F m %.3F %.3F l", $x1, $y1, $x2, $y2));
        if ($stroke) {
            $this->add_content(' S');
        }
    }
     ...
     * @param int $nSeg
     */
    public function part_ellipse($x0, $y0, $astart, $afinish, $r1, $r2 = 0, $angle = 0, $n_seg = 8): void
    {
        $this->ellipse($x0, $y0, $r1, $r2, $angle, $n_seg, $astart, $afinish, false);
    }
    /**
     ...
     * @param float $afinish
     */
    public function filled_ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $n_seg = 8, $astart = 0, $afinish = 360): void
    {
        $this->ellipse($x0, $y0, $r1, $r2, $angle, $n_seg, $astart, $afinish, true, true);
    }
    /**
     * @param float $x
     * @param float $y
     */
    public function line_to($x, $y): void
    {
        $this->add_content(sprintf("\n%.3F %.3F l", $x, $y));
    }
    /**
     ...
     * @param float $y3
     */
    public function curve_to($x1, $y1, $x2, $y2, $x3, $y3): void
    {
     ...
     * @param float $y
     */
    public function quad_to($cpx, $cpy, $x, $y): void
    {
     ...
     */
    public function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $n_seg = 8, $astart = 0, $afinish = 360, $close = true, $fill = false, $stroke = true, $incomplete = false): void
    {
        if ($r1 == 0) {
            return;
        }
     ...
     * @param int    $phase
     */
    public function set_line_style($width = 1, $cap = '', $join = '', $dash = '', $phase = 0): void
    {
        ...
        if ($string === $this->current_line_style) {
            return;
        }
        $this->current_line_style = $string;
        $this->add_content("\n{$string}");
    }
    /**
    ...
    public function polygon(array $p, bool $fill = false): void
    {
        ...
        $n = count($p);
        for ($i = 2; $i < $n; $i = $i + 2) {
            $this->add_content(sprintf('%.3F %.3F l ', $p[$i], $p[$i + 1]));
        }
        if ($fill) {
            $this->add_content(' f');
        } else {
     ...
     * @param float $height
     */
    public function filled_rectangle($x1, $y1, $width, $height): void
    {
     ...
     * @param float $height
     */
    public function rect($x1, $y1, $width, $height): void
    {
        $this->add_content(sprintf("\n%.3F %.3F %.3F %.3F re", $x1, $y1, $width, $height));
    }
    public function stroke(bool $close = false): void
    {
        $this->add_content("\n" . ($close ? 's' : 'S'));
    }
    public function fill(): void
    {
        $this->add_content("\nf" . ($this->fill_rule === 'evenodd' ? '*' : ''));
    }
    public function fill_stroke(bool $close = false): void
    {
        $this->add_content("\n" . ($close ? 'b' : 'B') . ($this->fill_rule === 'evenodd' ? '*' : ''));
    }
    /**
     ...
     * @return int
     */
    public function add_x_object($subtype, $x, $y, $w, $h)
    {
        $id = ++$this->num_obj;
     ...
     * @param array $options
     */
    public function set_x_object_resource($num_x_object, $type, $options): void
    {
        if (in_array($type, ['procset', 'font', 'xObject'])) {
            $this->o_xobject($num_x_object, $type, $options);
        }
    }
    /**
     * add signature
     ...
     * @return int
     */
    public function add_signature($signcert, $privkey, $password = '', $name = null, $location = null, $reason = null, $contactinfo = null)
    {
        $sig_id = ++$this->num_obj;
        $this->o_sig($sig_id, 'new', ['SignCert' => $signcert, 'PrivKey' => $privkey, 'Password' => $password, 'Name' => $name, 'Location' => $location, 'Reason' => $reason, 'ContactInfo' => $contactinfo]);
        return $sig_id;
    }
    /**
     * add field to form
     ...
     * @return int
     */
    public function add_form_field($type, $name, $x0, $y0, $x1, $y1, $ff = 0, $size = 10.0, $color = [0, 0, 0])
    {
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        $color = implode(' ', $color) . ' rg';
        $current_font_num = $this->current_font_num;
        $font = array_filter($this->objects[$this->current_node]['info']['fonts'], function (array $item) use ($current_font_num): bool {
            return $item['fontNum'] == $current_font_num;
        });
        $this->o_acroform($this->acro_form_id, 'font', ['objNum' => $font[0]['objNum'], 'fontNum' => $font[0]['fontNum']]);
        $field_id = ++$this->num_obj;
        $this->o_field($field_id, 'new', ['rect' => [$x0, $y0, $x1, $y1], 'F' => 4, 'FT' => "/{$type}", 'T' => $name, 'Ff' => $ff, 'pageid' => $this->current_page, 'da' => "{$color} /F{$this->current_font_num} " . sprintf('%.1F Tf ', $size)]);
        return $field_id;
    }
    /**
     * set Field value
     ...
     * @param string $value
     */
    public function set_form_field_value($num_field_obj, $value): void
    {
        $this->o_field($num_field_obj, 'set', ['value' => $value]);
    }
    /**
     ...
     */
    public function set_form_field_ref_value($num_field_obj, $num_obj): void
    {
        $this->o_field($num_field_obj, 'set', ['refvalue' => $num_obj]);
    }
    /**
     ...
     */
    public function set_form_field_appearance($num_field_obj, $normal_num_obj, $rollover_num_obj = null, $down_num_obj = null): void
    {
        $appearance['N'] = $normal_num_obj;
        if ($rollover_num_obj !== null) {
            $appearance['R'] = $rollover_num_obj;
        }
        if ($down_num_obj !== null) {
            $appearance['D'] = $down_num_obj;
        }
        $this->o_field($num_field_obj, 'set', ['appearance' => $appearance]);
    }
    /**
     ...
     * @param array $value
     */
    public function set_form_field_opt($num_field_obj, $value): void
    {
        $this->o_field($num_field_obj, 'set', ['options' => $value]);
    }
    /**
     ...
     */
    public function add_form($sig_flags = 0, $need_appearances = false): void
    {
        $this->acro_form_id = ++$this->num_obj;
        $this->o_acroform($this->acro_form_id, 'new', ['NeedAppearances' => $need_appearances ? 'true' : 'false', 'SigFlags' => $sig_flags]);
    }
    /**
    ...
    public function restore(): void
    {
        ...
        $this->current_stroke_color = null;
        $this->current_line_style = '';
        $this->current_line_transparency = null;
        $this->current_fill_transparency = null;
        $this->add_content("\nQ");
    }
    /**
     ...
     * @param float $height
     */
    public function clipping_rectangle($x1, $y1, $width, $height): void
    {
        $this->save();
     ...
     * @param float $rBL
     */
    public function clipping_rectangle_rounded($x1, $y1, $w, $h, $r_tl, $r_tr, $r_br, $r_bl): void
    {
        $this->save();
        // start: top edge, left end
        $this->add_content(sprintf("\n%.3F %.3F m ", $x1, $y1 - $r_tl + $h));
        // line: bottom edge, left end
        $this->add_content(sprintf("\n%.3F %.3F l ", $x1, $y1 + $r_bl));
        // curve: bottom-left corner
        $this->ellipse($x1 + $r_bl, $y1 + $r_bl, $r_bl, 0, 0, 8, 180, 270, false, false, false, true);
        // line: right edge, bottom end
        $this->add_content(sprintf("\n%.3F %.3F l ", $x1 + $w - $r_br, $y1));
        // curve: bottom-right corner
        $this->ellipse($x1 + $w - $r_br, $y1 + $r_br, $r_br, 0, 0, 8, 270, 360, false, false, false, true);
        // line: right edge, top end
        $this->add_content(sprintf("\n%.3F %.3F l ", $x1 + $w, $y1 + $h - $r_tr));
        // curve: bottom-right corner
        $this->ellipse($x1 + $w - $r_tr, $y1 + $h - $r_tr, $r_tr, 0, 0, 8, 0, 90, false, false, false, true);
        // line: bottom edge, right end
        $this->add_content(sprintf("\n%.3F %.3F l ", $x1 + $r_tl, $y1 + $h));
        // curve: top-right corner
        $this->ellipse($x1 + $r_tl, $y1 + $h - $r_tl, $r_tl, 0, 0, 8, 90, 180, false, false, false, true);
        // line: top edge, left end
        $this->add_content(sprintf("\n%.3F %.3F l ", $x1 + $r_bl, $y1));
        // Close & clip
        $this->add_content(' W n');
    }
    /**
     ...
     * @param float[] $p
     */
    public function clipping_polygon(array $p): void
    {
        $this->save();
        $this->add_content(sprintf("\n%.3F %.3F m ", $p[0], $p[1]));
        $n = count($p);
     ...
     */
    public function clipping_start(): void
    {
        $this->save();
    }
    /**
    ...
    {
        $y = $this->current_page_size['height'] - $y;
     ...
     */
    public function transform($tm): void
    {
     ...
     * @return int
     */
    public function new_page($insert = 0, $id = 0, $pos = 'after')
    {
        // if there is a state saved, then go up the stack closing them
        // then on the new page, re-open them with the right setings
        if ($this->n_state_stack) {
            for ($i = $this->n_state_stack; $i >= 1; $i--) {
        ...
        if ($insert) {
            ...
            $opt = ['rid' => $rid, 'pos' => $pos];
            $this->o_page($this->num_obj, 'new', $opt);
        } else {
            $this->o_page($this->num_obj, 'new');
        }
        // if there is a stack saved, then put that onto the page
        if ($this->n_state_stack) {
            for ($i = 1; $i <= $this->n_state_stack; $i++) {
                $this->save_state($i);
            }
        }
        // and if there has been a stroke or fill color set, then transfer them
        if (isset($this->current_color)) {
            $this->set_color($this->current_color, true);
        }
        if (isset($this->current_stroke_color)) {
            $this->set_stroke_color($this->current_stroke_color, true);
        }
        // if there is a line style set, then put this in too
        if ($this->current_line_style !== '') {
            $this->add_content("\n{$this->current_line_style}");
        }
     ...
     */
    public function stream($filename = 'document.pdf', array $options = []): void
    {
        if (headers_sent()) {
        ...
        $content_disposition = "Content-Disposition: {$attachment}; filename=\"{$fallbackfilename}\"";
        if ($fallbackfilename !== $filename) {
            $content_disposition .= "; filename*=UTF-8''{$encodedfilename}";
        }
     ...
     *
     */
    public function get_font_height(float $size): float
    {
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        $font = $this->fonts[$this->current_font];
    ...
    public function get_font_x_height(float $size): float
    {
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        ...
        if (isset($font['XHeight'])) {
            ...
            $xh = $this->get_font_height($size) / 2;
        }
        return $size * $xh / 1000;
    }
    /**
     ...
     *
     */
    public function get_font_descender(float $size): float
    {
        ...
        $h = $this->fonts[$this->current_font]['Descender'];
        return $size * $h / 1000;
    }
    /**
     ...
     */
    public function filter_text($text, $bom = true, $convert_encoding = true): string
    {
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        if ($convert_encoding) {
            $cf = $this->current_font;
            if (isset($this->fonts[$cf]) && $this->fonts[$cf]['isUnicode']) {
                $text = $this->utf8to_utf16be($text, $bom);
            } else {
                //$text = html_entity_decode($text, ENT_QUOTES);
                $text = mb_convert_encoding($text, self::$target_encoding, 'UTF-8');
            }
        } elseif ($bom) {
            $text = $this->utf8to_utf16be($text, $bom);
        }
     ...
     */
    public function utf8to_code_points_array(&$text): array
    {
     ...
     */
    public function utf8to_utf16be(&$text, $bom = true): string
    {
        $out = $bom ? "\xfe\xff" : '';
        $unicode = $this->utf8to_code_points_array($text);
     ...
     * @param $text
     */
    private function get_text_position($x, $y, $angle, float $size, $wa, string $text): array
    {
        // given this information return an array containing x and y for the end position as elements 0 and 1
        $w = $this->get_text_width($size, $text);
     ...
     *
     */
    public function to_upper(array $matches): string
    {
        return mb_strtoupper($matches[0], 'UTF-8');
    }
    public function concat_matches($matches): string
    {
        $str = '';
     ...
     * @param string $text
     */
    public function register_text($font, $text): void
    {
        if (!$this->is_unicode || in_array(mb_strtolower(basename($font), 'UTF-8'), self::$core_fonts)) {
            return;
        }
        if (!isset($this->string_subsets[$font])) {
            $base_subset = "�￾￿";
            // fffd => replacement character, fffe/ffff => not a character
            $this->string_subsets[$font] = $this->utf8to_code_points_array($base_subset);
        }
        $this->string_subsets[$font] = array_unique(array_merge($this->string_subsets[$font], $this->utf8to_code_points_array($text)));
    }
    /**
     ...
     */
    public function add_text($x, $y, $size, $text, $angle = 0, $word_space_adjust = 0, $char_space_adjust = 0, $small_caps = false): void
    {
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        ...
        if ($this->n_callback > 0) {
            for ($i = $this->n_callback; $i > 0; $i--) {
                // call each function
        ...
        if ($word_space_adjust != 0) {
            $this->add_content(sprintf(' %.3F Tw', $word_space_adjust));
        }
        if ($char_space_adjust != 0) {
            $this->add_content(sprintf(' %.3F Tc', $char_space_adjust));
        }
        if (strlen($text) > 0) {
            $part = $text;
            $place_text = $this->filter_text($part, false);
            // modify unicode text so that extra word spacing is manually implemented (bug #)
            if ($this->fonts[$this->current_font]['isUnicode'] && $word_space_adjust != 0) {
                $space_scale = 1000 / $size;
                $place_text = str_replace("\x00 ", "\x00 )\x00 " . -round($space_scale * $word_space_adjust) . "\x00 (", $place_text);
            }
            $this->add_content(" /F{$this->current_font_num} " . sprintf('%.1F Tf ', $size));
            $this->add_content(" [({$place_text})] TJ");
        }
        if ($word_space_adjust != 0) {
            $this->add_content(sprintf(' %.3F Tw', 0));
        }
        if ($char_space_adjust != 0) {
            $this->add_content(sprintf(' %.3F Tc', 0));
        }
        $this->add_content(' ET');
        ...
        if ($this->n_callback > 0) {
            for ($i = $this->n_callback; $i > 0; $i--) {
                // call each function
                $tmp = $this->get_text_position($x, $y, $angle, $size, $word_space_adjust, $text);
        ...
        if ($this->fonts[$this->current_font]['isSubsetting']) {
            $this->register_text($this->current_font, $text);
        }
    }
    /**
     ...
     *
     */
    public function get_text_width(float $size, string $text, float $word_spacing = 0.0, float $char_spacing = 0.0): float
    {
        static $ord_cache = [];
        // this function should not change any of the settings, though it will need to
        // track any directives which change during calculation, so copy them at the start
        // and put them back at the end.
        $store_current_text_state = $this->current_text_state;
        if (!$this->num_fonts) {
            $this->select_font($this->default_font);
        }
        ...
        if ($current_font['isUnicode']) {
            ...
            foreach ($unicode as $char) {
                ...
                if (isset($current_font['C'][$char])) {
                    ...
                    $char_width = $current_font['C'][0x20];
                }
            ...
            for ($i = 0; $i < $len; $i++) {
                $c = $text[$i];
                $char = $ord_cache[$c] ?? $ord_cache[$c] = ord($c);
                // check if we have to replace character
                if (isset($current_font['differences'][$char])) {
                    $char = $current_font['differences'][$char];
                }
        ...
        $this->current_text_state = $store_current_text_state;
        $this->set_current_font();
        return $w * $size / 1000;
    }
    /**
     ...
     * @param int $pageEnd
     */
    public function save_state($page_end = 0): void
    {
        if ($page_end) {
            // this will be called at a new page to return the state to what it was on the
            // end of the previous page, before the stack was closed down
            // This is to get around not being able to have open 'q' across pages
            $opt = $this->state_stack[$page_end];
            // ok to use this as stack starts numbering at 1
            $this->set_color($opt['col'], true);
            $this->set_stroke_color($opt['str'], true);
            $this->add_content("\n" . $opt['lin']);
            //    $this->currentLineStyle = $opt['lin'];
        } else {
            $this->n_state_stack++;
            $this->state_stack[$this->n_state_stack] = ['col' => $this->current_color, 'str' => $this->current_stroke_color, 'lin' => $this->current_line_style];
        }
        $this->save();
    }
    /**
     ...
     * @param int $pageEnd
     */
    public function restore_state($page_end = 0): void
    {
        if (!$page_end) {
            $n = $this->n_state_stack;
            $this->current_color = $this->state_stack[$n]['col'];
            $this->current_stroke_color = $this->state_stack[$n]['str'];
            $this->add_content("\n" . $this->state_stack[$n]['lin']);
            $this->current_line_style = $this->state_stack[$n]['lin'];
            $this->state_stack[$n] = null;
            unset($this->state_stack[$n]);
            $this->n_state_stack--;
        }
        $this->restore();
    }
    ...
    {
        $this->n_stack++;
        $this->stack[$this->n_stack] = ['c' => $this->current_contents, 'p' => $this->current_page];
        // add a new object of the content type, to hold the data flow
        $this->num_obj++;
        $this->o_contents($this->num_obj, 'new');
        $this->current_contents = $this->num_obj;
        $this->loose_objects[$this->num_obj] = 1;
        return $this->num_obj;
    }
    /**
     ...
     * @param $id
     */
    public function reopen_object($id): void
    {
        $this->n_stack++;
        $this->stack[$this->n_stack] = ['c' => $this->current_contents, 'p' => $this->current_page];
        $this->current_contents = $id;
    ...
    public function close_object(): void
    {
        ...
        if ($this->n_stack > 0) {
            $this->current_contents = $this->stack[$this->n_stack]['c'];
            $this->current_page = $this->stack[$this->n_stack]['p'];
            $this->n_stack--;
     ...
     * @param $id
     */
    public function stop_object($id): void
    {
        // if an object has been appearing on pages up to now, then stop it, this page will
        // be the last one that could contain it.
        if (isset($this->add_loose_objects[$id])) {
            $this->add_loose_objects[$id] = '';
        }
    }
    /**
     ...
     * @param string $options
     */
    public function add_object($id, $options = 'add'): void
    {
        // add the specified object to the page
        if (isset($this->loose_objects[$id]) && $this->current_contents != $id) {
            // then it is a valid object, and it is not being added to itself
            switch ($options) {
                case 'all':
                    // then this object is to be added to this page (done in the next block) and
                    // all future new pages.
                    $this->add_loose_objects[$id] = 'all';
                // no break
                case 'add':
                    if (isset($this->objects[$this->current_contents]['onPage'])) {
                        // then the destination contents is the primary for the page
                        // (though this object is actually added to that page)
                        $this->o_page($this->objects[$this->current_contents]['onPage'], 'content', $id);
                    }
                    break;
                case 'even':
                    $this->add_loose_objects[$id] = 'even';
                    $page_object_id = $this->objects[$this->current_contents]['onPage'];
                    if ($this->objects[$page_object_id]['info']['pageNum'] % 2 == 0) {
                        $this->add_object($id);
                        // hacky huh :)
                    }
                    break;
                case 'odd':
                    $this->add_loose_objects[$id] = 'odd';
                    $page_object_id = $this->objects[$this->current_contents]['onPage'];
                    if ($this->objects[$page_object_id]['info']['pageNum'] % 2 == 1) {
                        $this->add_object($id);
                        // hacky huh :)
                    }
                    break;
     ...
     * @param $id
     */
    public function serialize_object($id): ?string
    {
     ...
     * @return int
     */
    public function restore_serialized_object($obj)
    {
        $obj_id = $this->open_object();
     ...
     */
    public function add_embedded_file(string $filepath, string $embedded_filename, string $description, string $mime_type = 'application/octet-stream', array $related_to = []): void
    {
        $ef_dict_id = ++$this->num_obj;
        $this->o_embedded_file_dictionary($ef_dict_id, 'new', ['filepath' => $filepath, 'filename' => $embedded_filename, 'description' => $description, 'mimeType' => $mime_type]);
        foreach ($related_to as $obj_id => $relationship) {
            $this->associate_file($ef_dict_id, $obj_id, $relationship);
        }
    }
    /**
     ...
     */
    public function associate_file(int $file_id, int $object_id, ?string $relationship = 'Unspecified'): void
    {
        $relationship = $relationship ?? 'Unspecified';
        $obj = $this->objects[$object_id];
        $type = 'o_' . $obj['t'];
        $this->{$type}($object_id, 'associatedFiles', "{$file_id} 0 R");
        $this->o_embedded_file_dictionary($file_id, 'afRelationship', $relationship);
    }
    /**
     ...
     */
    public function add_info($label, string $value = ''): void
    {
        ...
        if (is_array($label)) {
            foreach ($label as $l => $v) {
                $this->o_info($this->info_object, $l, (string) $v);
            }
        } else {
            $this->o_info($this->info_object, $label, $value);
        }
    }
    /**
     ...
     * @param int $value
     */
    public function set_preferences($label, $value = 0): void
    {
        // this will only work if the label is one of the valid ones.
        if (is_array($label)) {
            foreach ($label as $l => $v) {
                $this->o_catalog($this->catalog_id, 'viewerPreferences', [$l => $v]);
            }
        } else {
            $this->o_catalog($this->catalog_id, 'viewerPreferences', [$label => $value]);
        }
    }
    /**
     ...
     * @param $num
     */
    private function get_bytes(array &$data, $pos, int $num): int
    {
     ...
     */
    public function image_iscached($imgname): bool
    {
     ...
     * @throws Exception
     */
    public function add_image_png(&$img, string $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null): void
    {
        ...
        if (isset($this->imagelist[$file])) {
            ...
            if ($error) {
                $this->add_message('PNG error - (' . $file . ') ' . $errormsg);
                return;
            }
        }
        //End isset($this->imagelist[$file]) (png Duplicate removal)
        $this->add_png_from_buf($data, $file, $x, $y, $w, $h, $is_mask, $mask);
    }
    /**
     * @param $file
     * @param $x
     ...
     * @param $byte
     */
    protected function add_image_png_alpha($file, $x, $y, $w, $h, $byte)
    {
        // generate images
        ...
        if (extension_loaded('gmagick')) {
            ...
            static $imagick_clonable = null;
            if ($imagick_clonable === null) {
                $imagick_clonable = true;
                if (defined('Imagick::IMAGICK_EXTVER')) {
                    $imagick_version = \Imagick::IMAGICK_EXTVER;
                } else {
                    $imagick_version = '0';
                }
                if (version_compare($imagick_version, '0.0.1', '>=')) {
                    $imagick_clonable = version_compare($imagick_version, '3.0.1rc1', '>=');
                }
            }
            $imagick = new \Imagick();
            $imagick->set_registry('temporary-path', $this->tmp);
            $imagick->set_format('PNG');
            $imagick->read_image($file);
            // Get opacity channel (negative of alpha channel)
            if ($imagick->get_image_alpha_channel()) {
                $alpha_channel = $imagick_clonable ? clone $imagick : $imagick->clone();
                $alpha_channel->separate_image_channel(\Imagick::CHANNEL_ALPHA);
                // Since ImageMagick7 negate invert transparency as default
                if (\Imagick::get_version()['versionNumber'] < 1800) {
                    $alpha_channel->negate_image(true);
                }
                try {
                    $alpha_channel->write_image($tempfile_alpha);
                } catch (\Imagick_Exception $th) {
                    // Backwards compatible retry attempt in case the IMagick policy is still configured in lowercase
                    $alpha_channel->set_format('png');
                    $alpha_channel->write_image($tempfile_alpha);
                }
                // Cast to 8bit+palette
                $imgalpha_ = @imagecreatefrompng($tempfile_alpha);
            ...
            $color_channels->set_registry('temporary-path', $this->tmp);
            $color_channels->new_image($wpx, $hpx, '#FFFFFF', 'png');
            $color_channels->composite_image($imagick, \Imagick::COMPOSITE_COPYRED, 0, 0);
            $color_channels->composite_image($imagick, \Imagick::COMPOSITE_COPYGREEN, 0, 0);
            $color_channels->composite_image($imagick, \Imagick::COMPOSITE_COPYBLUE, 0, 0);
            $color_channels->write_image($tempfile_plain);
            $imgplain = @imagecreatefrompng($tempfile_plain);
        } else {
            ...
            for ($xpx = 0; $xpx < $wpx; ++$xpx) {
                for ($ypx = 0; $ypx < $hpx; ++$ypx) {
                    ...
                    if ($eight_bit) {
                        ...
                        $gammacorr = 2.2;
                        $pixel = round(((127 - $alpha) * 255 / 127 / 255) ** $gammacorr * 255);
                    } else {
        ...
        $this->image_alpha_list[$file] = [$tempfile_alpha, $tempfile_plain];
        // embed mask image
        if ($tempfile_alpha) {
            $this->add_image_png($imgalpha, $tempfile_alpha, $x, $y, $w, $h, true);
            if (PHP_MAJOR_VERSION < 8) {
                imagedestroy($imgalpha);
            }
            $this->image_cache[] = $tempfile_alpha;
        }
        // embed image, masked with previously embedded mask
        $this->add_image_png($imgplain, $tempfile_plain, $x, $y, $w, $h, false, $tempfile_alpha !== null);
        if (PHP_MAJOR_VERSION < 8) {
            imagedestroy($imgplain);
        }
        $this->image_cache[] = $tempfile_plain;
    }
    /**
     ...
     * @throws Exception
     */
    public function add_png_from_file($file, $x, $y, $w = 0, $h = 0): void
    {
        if (!function_exists('imagecreatefrompng')) {
            throw new \Exception('The PHP GD extension is required, but is not installed.');
        }
        if (isset($this->image_alpha_list[$file])) {
            [$alpha_file, $plain_file] = $this->image_alpha_list[$file];
            if ($alpha_file) {
                $img = null;
                $this->add_image_png($img, $alpha_file, $x, $y, $w, $h, true);
            }
            $img = null;
            $this->add_image_png($img, $plain_file, $x, $y, $w, $h, false, $plain_file !== null);
            return;
        }
        //if already cached, need not to read again
        if (isset($this->imagelist[$file])) {
            ...
            if ($is_alpha) {
                ...
                $this->add_image_png_alpha($file, $x, $y, $w, $h, $color_type);
                return;
            }
        ...
        $this->add_image_png($img, $file, $x, $y, $w, $h);
     ...
     * @param bool $is_mask
     */
    public function add_png_from_buf(&$data, string $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null): void
    {
        if (isset($this->imagelist[$file])) {
            $data = null;
            ...
            if ($data == null) {
                $this->add_message('addPngFromBuf error - data not present!');
                return;
            }
            $error = 0;
            ...
            if (!$error) {
                ...
                $pdata = '';
                while ($p < $len) {
                    $chunk_len = $this->get_bytes($data, $p, 4);
                    $chunk_type = mb_substr($data, $p + 4, 4, '8bit');
                    switch ($chunk_type) {
                        case 'IHDR':
                            // this is where all the file information comes from
                            $info['width'] = $this->get_bytes($data, $p + 8, 4);
                            $info['height'] = $this->get_bytes($data, $p + 12, 4);
                            ...
                            break;
                        case 'PLTE':
                            $pdata .= mb_substr($data, $p + 8, $chunk_len, '8bit');
                            break;
                        case 'IDAT':
                            $idata .= mb_substr($data, $p + 8, $chunk_len, '8bit');
                            break;
                        case 'tRNS':
                            ...
                            switch ($info['colorType']) {
                                ...
                                case 2:
                                    /* corresponding to entries in the plte chunk
                                       ...
                                       */
                                    $transparency['r'] = $this->get_bytes($data, $p + 8, 2);
                                    // r from truecolor
                                    $transparency['g'] = $this->get_bytes($data, $p + 10, 2);
                                    // g from truecolor
                                    $transparency['b'] = $this->get_bytes($data, $p + 12, 2);
                                    // b from truecolor
            ...
            $this->o_image($this->num_obj, 'new', $options);
     ...
     * @param int $h
     */
    public function add_jpeg_from_file($img, $x, $y, $w = 0, $h = 0): void
    {
        ...
        if ($this->image_iscached($filename)) {
            $data = null;
            $image_width = $this->imagelist[$filename]['w'];
            $image_height = $this->imagelist[$filename]['h'];
        ...
        if ($w == 0) {
            $w = $h / $image_height * $image_width;
        }
        if ($h == 0) {
            $h = $w * $image_height / $image_width;
        }
        $this->add_jpeg_image_common($data, $filename, $image_width, $image_height, $x, $y, $w, $h, $channels);
    }
    /**
     ...
     * @param int $channels
     */
    private function add_jpeg_image_common(&$data, string $imgname, $image_width, $image_height, $x, $y, $w = 0, $h = 0, $channels = 3): void
    {
        if ($this->image_iscached($imgname)) {
            ...
            if ($data == null) {
                $this->add_message('addJpegImage_common error - (' . $imgname . ') data not present!');
                return;
            }
            ...
            $label = "I{$im}";
            $this->num_obj++;
            $this->o_image($this->num_obj, 'new', ['label' => $label, 'data' => &$data, 'iw' => $image_width, 'ih' => $image_height, 'channels' => $channels]);
            $this->imagelist[$imgname] = ['label' => $label, 'w' => $image_width, 'h' => $image_height, 'c' => $channels];
        }
        $this->add_content(sprintf("\nq\n%.3F 0 0 %.3F %.3F %.3F cm /%s Do\nQ ", $w, $h, $x, $y, $label));
    }
    /**
     ...
     * @param int $c
     */
    public function open_here($style, $a = 0, $b = 0, $c = 0): void
    {
        ...
        // 'FitBV' left
        $this->num_obj++;
        $this->o_destination($this->num_obj, 'new', ['page' => $this->current_page, 'type' => $style, 'p1' => $a, 'p2' => $b, 'p3' => $c]);
        $id = $this->catalog_id;
        $this->o_catalog($id, 'openHere', $this->num_obj);
    }
    /**
     * Add JavaScript code to the PDF document
     */
    public function add_javascript(string $code): void
    {
     ...
     * @param int $c
     */
    public function add_destination($label, $style, $a = 0, $b = 0, $c = 0): void
    {
     ...
     * @param string $options
     */
    public function set_font_family($family, $options = ''): void
    {
        if (!is_array($options)) {
            if ($family === 'init') {
                ...
                $this->font_families['Helvetica.afm'] = ['b' => 'Helvetica-Bold.afm', 'i' => 'Helvetica-Oblique.afm', 'bi' => 'Helvetica-BoldOblique.afm', 'ib' => 'Helvetica-BoldOblique.afm'];
                $this->font_families['Courier.afm'] = ['b' => 'Courier-Bold.afm', 'i' => 'Courier-Oblique.afm', 'bi' => 'Courier-BoldOblique.afm', 'ib' => 'Courier-BoldOblique.afm'];
                $this->font_families['Times-Roman.afm'] = ['b' => 'Times-Bold.afm', 'i' => 'Times-Italic.afm', 'bi' => 'Times-BoldItalic.afm', 'ib' => 'Times-BoldItalic.afm'];
            }
        } else if (isset($family) && mb_strlen($family)) {
            $this->font_families[$family] = $options;
        }
    }
    /**
